...
- **Sources d'or exogènes** (toutes créditées à `treasury`) :
  ...
  - **Sac de ville** : capturer une ville ennemie crédite le captreur d'une **part de la trésorerie du perdant** — `economy.json` `cityCapturePlunderPct` 🔶 **0.5** (sources muettes — montant calibrable, arrondi au plus proche, minimum 0 ; la capitale terminant la partie, le sac s'applique aux villes secondaires) ; champ `plunder` ajouté à l'événement `CityCaptured`.
...
**R-135 · Rush-buy (achat instantané).** Coût en or = **marteaux restants × facteur d'ère** (strictement linéaire ; 0 marteau investi = coût total × facteur) : `coût = round(max(0, coût_item − progression) × facteur(ère) × réductions)` (round half up). L'**ère** est celle de l'empire (la plus avancée des technologies débloquées — même définition que la pop de fondation D3/R-64). Facteurs **data-driven** (`economy.json` `eraRushFactors`) 🔶 ancrés sur les exemples chiffrés du doc : **Antique ×2** (Guerrier 10 → ~20-30 ; Pyramides 150 → 300-450), **Médiévale ×3** (Marché 60 → 180 ; Banque 120 → 360 ; Pyramides 150 → 450), **Industrielle ×5** 🔶 **proposé** (aucun exemple exact dans le doc — à calibrer), **Moderne ×8** (Bibliothèque 40 → 320 ; Banque 120 → 960).
...
Table **data-driven** (`economy.json` `milestones`). Les bâtiments gratuits suivent R-66/R-111 (déjà possédé = saute ; remplacement applicable) ; les citoyens des villes grossies sont auto-assignés (R-60). Le GP des canaux or est posé à la **capitale** (sinon première ville — canon « apparaît aux abords de la capitale »), classe par ciblage technologique R-127 (défaut de canon 🔶, déterministe) et **sans jalon** (miroir C2 — seuls les GP du canal culture comptent). Chaque palier émet `EconomyMilestone { player, threshold, reward, label }` (libellés FR) + les événements d'effet concrets (`UnitProduced`, `TechResearched`, `GreatPersonSpawned`, `BuildingCompleted`, `PopulationGrew`).
...
**R-150 · Avantages de départ (setup déterministe).** `createInitialState(map, seed, civSetup?)` applique, dans l'ordre : techs de départ (octroi direct, sans événement) → gouvernement de départ (sans Anarchie) → bâtiments gratuits de la capitale → Merveille Antique de l'Égypte (liste fermée, validée) → or de départ → GP gratuit (Amérique, classe R-127 rotation — sans jalon) → révélation de carte (Russie) → **remplacement des unités de départ** par les uniques disponibles (R-148) → vétérans de trait (Allemagne). Le serveur construit `civSetup` depuis `meta.players` (choix au lobby — hôte à la création, invité au join) ; la même graine + les mêmes civs → le même état (idempotent, crash-recovery §3.5).
