...
| Case de ville | oui | +50 % (T-02) | 1/1/1 | — | — |
| Cratère *(7n — C15)* | oui | 0 % | **0/0/0 (stérile)** | — | — |

- **Cratère (7n · C15)** : formé par l'annihilation nucléaire d'une ville ordinaire — la case (ex-case de ville) devient un terrain `cratere` stérile (rendements nuls, ressource effacée) et **non fondable** ; défaut 🔶 **permanent** (le canon est muet sur une réutilisation — alternative : réutilisable après N tours, données à venir). Les tirs sur des cases quelconques hors ville ne créent PAS de cratère.
...
**R-184 · Blocages de fin de tour — FIN-DE-TOUR-PRODUCTION (spécification d'Erik du 18/09).** Le joueur **ne peut pas finaliser son tour** tant qu'il laisse des capacités sans emploi — prédicat pur `blocagesFinDeTour(state, playerId, orders)` (`packages/rules/src/finDeTour.ts`, source unique consommée par l'UI **ET** par la validation serveur du `EndTurn`, rejet = `OrderAck` négatif avec la liste des blocages) :
- **Production** : une ville qui **génère des marteaux** (production/tour > 0 — citoyen intérieur et cases travaillées comprises) **ou** qui porte un **résiduel de marteaux** (réserve permanente C7 `pendingSalvage`) doit avoir **une production sélectionnée** — l'état de la ville (`city.production`) **ou un brouillon `SetProduction` en file** suffit (les brouillons ne sont appliqués qu'à la résolution ; le prédicat les lit, miroir de l'aperçu UI). Une ville à 0 marteau sans réserve n'est **jamais** bloquée.
- **Recherche** : si le joueur **produit de la science** (science/tour > 0) **ou** a des **points de recherche non utilisés** (réserve `scienceStored` — ex. bonus de hutte recherche sans sélection, R-85), il doit avoir **une recherche sélectionnée**. **R-134 CONFIRMÉE (Erik, 18/09)** : le surplus de recherche à la complétion d'une tech reste converti **1:1 en or** — il ne devient JAMAIS un résiduel ; le résiduel de fin de tour est exclusivement la réserve `scienceStored`. **Exception** : arbre de recherche **épuisé** (plus aucune tech disponible) → le blocage recherche est levé.
- Le blocage est **PRÉ-résolution** (phase « orders ») ; les unités sans ordre restent régies par le dialogue UI existant (hors moteur). Le **bot** est soumis aux mêmes règles : son plan (`botPolicy`) sélectionne recherche et productions — aucun blocage ne doit subsister après son plan (testé). UI : bouton « Fin de tour » désactivé avec la liste des blocages (« Ville1 : sélectionnez une production », « Recherche : sélectionnez une technologie (+N points en attente) »), avertissements dans la vue ville et le panneau de recherche. **Aucune migration `schemaVersion`** : le résiduel est le champ existant `scienceStored` (v25 inchangée).

## 4bis. Programmation des déplacements — DEPLACEMENT-PLANIFIE (ajouté le 06/09/2026, décisions D1–D6 d'Erik)

**R-158 · Ordre multi-étapes (D5).** Nouvel ordre **`MultiStep { unitId, path, final? }`** — enchaîne **déplacement(s)** (le `path` suit exactement les règles R-40..R-43) puis **UNE action finale**, dans la limite des PM (ex. Colon : 1 case puis fonder). Hors périmètre : action PUIS re-mouvement. L'action finale (`foundCity` — catalogue data-driven `deplacement.json` `multiStepFinalActions`) s'exécute en Phase C si l'unité a atteint le terme du chemin, est vivante, n'est PAS entrée dans le fog ce tour (R-161) et dispose d'au moins `mpCostOfFinalAction` 🔶 (1) PM — sinon elle est **annulée sans erreur** et le mouvement (partiel ou complet) est conservé. L'**attaque après déplacement** n'est pas une étape finale : le dernier pas du chemin SUR la case de l'ennemi déclenche le combat d'entrée existant (R-42/D4). Un ordre multi-étapes qui échoue en cours de route (blocage amical, chemin invalide — la halte X-2 est abrogée, voir table X-2) exécute ce qui peut l'être et s'arrête — jamais de crash ni d'ordre fantôme ; le chemin gelé conserve la forme composite. Le **bot** reste aux ordres simples : `Move` reste accepté en entrée pour toujours (compat composite). **Migration `schemaVersion` 18 → 19** : les chemins gelés persistés (`unit.order` de type `Move`) sont normalisés en composites à une étape — sans perte, idempotent ; une partie pré-19 reprend sans erreur.
...
- **R-159 rév. C — cohabitation transitoire de départ (RESOLUTION-DEPLACEMENTS, Erik 18/09).** L'entrée sur une case portant des amies est AUTORISÉE quand **toutes** les amies présentes programment de quitter la case ce tour (ordre Move/MultiStep actif dont le premier pas est ailleurs que la case de l'entrant) : la cohabitation n'est que TRANSITOIRE (Phase A) et l'ordre unitId (R-41) ne crée plus de fenêtre de refus. **L'échange de cases reste interdit** : si une amie part vers la case de l'entrant (premier pas = case d'origine de l'entrant), l'entrée est refusée comme avant (bug 2 — message client « Deux unités ne peuvent pas interchanger de position »). Si un départ programmé échoue finalement, la cohabitation ami/ami de fin de tour est résolue par la dispersion (R-179) et la fondation obéit à **l'option B** (fondation ANNULÉE si une amie cohabite encore sur la case à l'arrivée — arbitrage Erik 18/09 ; le colon cohabite et fondera au tour suivant ; décision de trace `fondation-annulee-cohabitation`).
...
- **choix du défenseur (rév. DÉFENSE-DE-PILE — décision Erik du 15/09)** : lorsqu'une case compte **plusieurs unités** (pile de camp barbare R-96, et toute future co-location), l'unité qui **se défend** est celle à la **plus grande force de défense** (stat de base du type — canon CivRev : guerrier 1, archer 2, piquier 3, fusilier 5, infanterie moderne 8, cuirasse 18, …) ; à **défense égale**, celle avec le **plus de PV courants** ; dernier tie-break R-81 (`unitId` croissant). Chaque unité tuée est remplacée par la suivante (assaut un par un) ;
...
### 7.9 Barbares & huttes (Phase 7d — ajouté le 01/09/2026, transcription de [`HANDOFF-PHASE7D.md`](docs/historique/handoffs/HANDOFF-PHASE7D.md))
...
**R-97 · IA barbare (déterministe, rév. BARBARES-PILES).** `barbarianOrders(state)` — fonction pure, appelée **en tête de `resolveTurn`**, dont les ordres suivent les phases normales (mouvements → combats ; l'économie ne la concerne pas). Ordre de priorité **par unité** (unités triées R-81) :
1. **attaquer** une unité ou ville ennemie **adjacente** (case à défenseur = attaque du défenseur, R-57 ; ordre `Attack`) ;
2. sinon **avancer d'un pas** vers l'entité ennemie la plus proche dans un rayon d'aggro `T-19` 🔶 = **2 cases** (POLISSAGE-1 C3 : les barbares ne sortent du camp que si une unité ennemie est à 2 cases ou moins ; première case de la ligne hexagonale ; case injoignable/occupée amicalement → tenir) ;
3. sinon **tenir** (`Hold`).
**Garde minimale `T-49` = 2 (rév. BARBARES-PILES 15/09 — était 1)** : les barbares d'un camp forment une **pile** sur sa case ; les **`gardeMinimale` PREMIERS barbares vivants** dans l'**ordre d'engendrement** (`spawnedUnits` — ordre des ids, R-81 : **le plus ancien garde, le dernier arrivé sort**) ne **SORTENT JAMAIS** (pas de `Move`) ; **seul le dernier arrivé** (au plus 1 avec cap 3 et garde 2) explore — une pile de 1 ou 2 ne sort donc **personne**. Interprétation 🔶 documentée : la garde contraint les **sorties** (mouvements), pas le combat défensif — une garde attaque toujours un ennemi **adjacent**. Tie-breaks R-81 partout (distance, puis `(q, r)`). **Les barbares ne subissaient pas la halte X-2 (abrogée le 18/09 — plus de halte du tout)** : leurs ordres (un pas) sont régénérés à chaque résolution. Les barbares **peuvent capturer les villes sans défenseur** (R-57/R-65) : 🔶 **la ville est alors rasée** (événement `CityRazed` — elle disparaît, bâtiments perdus, aucun changement de propriétaire) — **si la capitale d'un joueur est rasée, ce joueur perd** : événement `Victory(reason:'razedCapital')` au profit de l'adversaire réel (les barbares ne gagnent jamais).
...
**R-99 · Données de calibrage.** Toutes les constantes barbares/huttes vivent dans **`barbares.json`** (`spawnInterval`, `aggroRadius`, `gardeMinimale` (T-49), `initialUnits` (T-50), `capPerVillage`, `escalationTurn`, `units` d'escalade — rév. BARBARES-PILES : `villageDestructionGold`, `villageHP` et `villageDefense` **supprimés**) et **`huttes.json`** (table de récompenses pondérée) — **zéro durcissement de règle dans le code** : calibrer = éditer le JSON + push (CI déploie), même philosophie que R-86/R-91. `constants.ts` ré-exporte les valeurs (source unique des T-18..T-26 et T-49/T-50 côté code). Tests d'intégrité : table de récompenses fermée (kinds connus), poids ≥ 0 et somme > 0, bornes or cohérentes (`min ≤ max`), unités d'escalade existantes.
...
Base documentaire : la spécification d'Erik [`Artefacts Dans Civilization Revolution.md`](docs/recherche/Artefacts Dans Civilization Revolution.md) — **elle fait foi**. **Complément de calibrage canon : [`ArtefactsdeCivilization Revolution.md`](docs/recherche/ArtefactsdeCivilization Revolution.md) (Erik, 06/09)** — tirage 4-5, distance capitales 8, indice 15 %, Confucius 2 GP hors escalade, Templiers toujours Vétérane. Sources croisées : [CivFanatics — Relics](https://civfanatics.com/civrev/civilopedia/relics/), [Civilization Wiki — Artifacts (CivRev)](https://civilization.fandom.com/wiki/Artifacts_(CivRev)). **Périmètre canon : les 6 artefacts du jeu de base** ; les 6 DLC (Camelot, Sphinx, Aiguille, Terracotte, Rayon de Tesla, Babel) sont exclus v1 (`dlcOnly: true` en données, jamais générés).

**Décisions de tranche (pilotage 7o) — RÉVISÉES par le calibrage canon (rapports « Artefacts de Civilization Revolution » / « Calibrage Mécaniques » / « Rules Research », Erik 06/09)** : tirage **4-5** artefacts par carte (canon dataminé : entre 4 et 5, jamais la réserve complète) ; **Atlantide toujours dans le tirage** 🔶 (canon : « presque systématiquement » — défaut : toujours) ; l'or des Sept Cités suit le multiplicateur `tresorsDouble` espagnol (hook R-146 déjà posé en 7n) ; les merveilles à condition dynamique (Nations Unies, Banque mondiale) et stratégique (Projet Manhattan) **ne sont pas choisibles via Angkor** 🔶 (un octroi gratuit contournerait leurs verrous R-116/R-137/R-138) ; l'indice de hutte est **une seule entrée de table** dont l'effet (nombre restant **ou** position) est tiré au RNG à **15 %** (canon 15-20 %, borne basse) ; le survol d'une case masquée portant un artefact expose un **marqueur de présence discret** 🔶 (le canon décrit un bourdonnement sonore — l'audio est différé : défaut lueur au survol, présence seule, jamais l'identité).

**R-151 · Tirage.** À la création de la carte (procédurale **et** préfabriquée), un tirage **sans remise** de `count` (**4**, borné **4–5** — `artefacts.json` `countMin`/`countMax` ; calibrage canon : entre 4 et 5 artefacts générés, jamais la réserve complète) artefacts parmi le **pool de 6** ; RNG **dédié dérivé du seed de partie** (R-80 — même seed → même tirage et même placement, rejouable ; le tirage ne consomme PAS le RNG de résolution) ; chaque artefact est **unique** sur la carte et **disparaît définitivement** à son activation. L'**Atlantide fait partie du tirage** 🔶 (défaut : toujours) ; les autres artefacts sont tirés sans remise parmi les 5 restants.
...
- **artefacts terrestres** : priorité aux **îles isolées / atolls** — cases de terre d'une composante connexe (BFS) de taille ≤ `islandMaxSize` 🔶 (5), à distance ≥ `minDistanceToCapitals` **8** (calibrage canon : 8-10 cases des capitales, borne basse) des **deux** départs ; à défaut, cases du continent principal **les plus éloignées** des départs — au plus `maxMainland` 🔶 (2) artefacts continentaux (rare, canon « 1 à 2 possibles » ; sauf si le plancher canon de 4 artefacts l'exige — carte sans îles candidates) ;
...
| **École de Confucius** | **2 GP gratuits** (canon dataminé) — classes par **rotation déterministe R-80** (indices `greatPersonsObtained` + i — le ciblage R-127 est abrogé), posés à la **capitale** (sinon première cité — case libre adjacente), **sans jalon** (miroir C2). **EXEMPTION D'ESCALADE (révision Calibrage canon — Erik 06/09)** : les GP de Confucius sont une **manne hors progression** — ils N'incrémentENT ni `greatPersonsObtained` (seuil culturel T-27) ni les compteurs par type ; le seuil culturel des GP suivants est **inchangé** (canon : « cet octroi n'augmente pas le coût culturel requis pour les GP suivants »). |
| **Chevaliers Templiers** | Une **unité militaire selon l'Ère** — Ancienne/Médiévale → **Chevalier**, Industrielle → **Canon**, Moderne → **Char d'assaut** (table data-driven) ; **l'unité arrive TOUJOURS VÉTÉRANE (5 XP)** (calibrage canon — Erik 06/09) ; remplacement par l'**unique de la civ** (R-148) ; posé sur la case de l'artefact (occupée par l'activateur → première case adjacente libre — perdu si aucune). |
...
**R-63 · Croissance (révision ALIGNEMENT-CROISSANCE — Erik 13/09, d'après le vrai jeu ; révisions antérieures : 7l C4, 7i D1/D2, 30/08, v1).**
...
- **D5 — Fonder sur une ressource la DÉTRUIT** définitivement (la case devient une case de ville, la ressource est effacée) ; événement **`ResourceDestroyed`** 🔶 (libellé à calibrer) ; l'UI avertit avant de fonder sur une case à ressource visible. **Non-régression testée** (révision centre-ville R-66 du 06/09).
...
- **R-66 (rév. 06/09 — centre-ville) ABROGÉE le 13/09 (ALIGNEMENT-CROISSANCE, Erik d'après le vrai jeu)** — la case de ville ne produit **RIEN** : **0 Nourriture, 0 Production, 0 Commerce** (terrain `ville` = 0/0/0 dans `terrain.json`, le socle/plancher `cityCenter.floor` est supprimé, le champ disparaît de `GrowthData`). La ville vit **exclusivement** par ses tuiles travaillées (R-60) et ses **citoyens intérieurs** R-88 (qui conservent leur production/tranche commerce). Le centre reste **travaillé d'office** (non assignable, R-60) mais sans rendement propre ; traits de civ et bâtiments continuent de s'appliquer aux **autres** terrains. Historique du socle : « 1 P seul » (7i) → « 1N/1P/1C » (rév. 06/09) → **0/0/0** (13/09).
...
**Base : [`RECHERCHE-RESSOURCES.md`](docs/recherche/RECHERCHE-RESSOURCES.md) §2** (données officielles CivRev sourcées) — les valeurs ci-dessous sont la table officielle. Tout est **éditable en données** (`resources.json`, même philosophie relationnelle que R-86) : le scénario étalon « déplacer Gemmes de montagne à colline » = éditer son champ `terrains`, rien d'autre. Champs : `{id, name, terrains[], yields{N/P/C}, revealedByTech, officialTech, culture, hiddenUntilRevealed, spawnWeight}`.
...
Base documentaire : la spécification d'Erik [`Culture dans Civilization Revolution.md`](docs/recherche/Culture dans Civilization Revolution.md). Constantes culturelles : `culture.json` (calibrage par édition, même philosophie que barbares.json R-99) ; coût des merveilles : `wonders.json`. **Décisions de tranche (pilotage 7f)** : la **conversion culturelle passive est REPORTÉE** (elle exige un concept de territoire — 7g/7h) ; les GP couverts ici sont ceux de la **culture** (Artiste, Penseur — les GP or/science/production/combat viendront en 7g/7h) ; **trois merveilles à effets simples sont activées** (sinon les 20 jalons seraient inatteignables avec les seuls GP).

**R-113 · Rendement culturel (rév. EXPANSION-CULTURELLE, 13/09).** Par ville et par tour : **Palais** — révisé (décision d'Erik) : **+1 culture par citoyen, plafonné aux 5 premiers niveaux de population** (`min(pop, 5)`, données `buildings.json` `palais.culturePerCitizenCap { perCitizen: 1, cap: 5 }` 🔶 — capitale uniquement, le Palais n'existe que là ; à pop 6+, 5 culture/tour) + Σ bâtiments `culturePerCitizen` × **population** (Temple +1/citoyen, Cathédrale +2/citoyen — R-111 : elle remplace le Temple, **non plafonnés**) + le bonus empire `perCity.culture` du Premier découvrir (R-109 : Religion et Imprimerie +1 — activées en 7f). **Interactions verrouillées par tests** : la Monarchie (R-121) double la part Palais SEULE ; le Communisme annule Temples/Cathédrales mais PAS le Palais ; **Stonehenge** multiplie par 🔶 1,5 la part `culturePerCitizen` (lignée Temple/Cathédrale) tant qu'il n'est pas obsolète (Littératie, R-110) mais PAS le Palais ; Magna Carta (Tribunal +1/citoyen, R-133) et Théâtre (×2 total, R-132) inchangés. La culture est **scalaire sur la démographie** (20 pop × Cathédrale = 40 🔶, exemple du document d'Erik). Accumulée **par ville** dans le CUMUL `city.cultureCumulee` — **GP-CULTURE-EVENEMENTS (rév. D1/D5, décision d'Erik du 13/09) : l'ancien réservoir consommé `cultureStored` est SUPPRIMÉ de l'état (migration 22) ; la culture n'est JAMAIS soustraite**, le canal GP lit le cumul EMPIRE (R-114 rév. D1). Les ressources à culture (Encens/Soie, D2) restent ignorées (en suspens, 7g/7h). 🔶 pacing : la révision accélère le canal GP — les seuils T-27 (`culture.json`) sont le levier de calibrage séparé.

**R-162 · Anneaux culturels (phase 1 — visual-only, ajouté le 13/09).** Chaque ville accumule **`city.cultureCumulee`** (somme des mêmes gains que `cultureStored`, **jamais consommé** — le canal GP soustrait `cultureStored` seul ; figée en Anarchie R-122 comme `cultureStored`). `rayonCulturelDe` (pure) = nombre d'anneaux franchis : **+1 anneau par seuil de T-51** (10 / 100 / 1 000 / 10 000 🔶, `culture.json` `cultureExpansionThresholds`), **plafond T-52 = 5** (`cultureExpansionMaxRings`), 0 au départ (la zone = les tuiles cultivées seules). **Phase 1 : AUCUN effet gameplay** — les anneaux sont une indication visuelle (rendu 2D : bande d'extension autour de la zone cultivée, liseré/dégradé sur la frontière extérieure seule, clippée au fog) ; `workRadiusOf`/`tileWorkable`/worked tiles/conversions n'y touchent pas. Le concept de territoire réel (conversion, flip, Remparts) reste au backlog. **Migration `schemaVersion` 19 → 20** : champ additif `cultureCumulee`, backfill `0`, idempotent. **RÉV. CULTURE-FRONTIERES (14/09 — décision d'Erik) : la frontière progresse PALIER APRÈS PALIER** — palier 0 : aucun liseré ; palier 1 : le liseré accent joueur entoure la ZONE CULTIVÉE seule (contour des worked tiles — frontière à rayon 1, jamais 2, Tribunal compris) ; palier 2 et plus : la bande d'extension s'ajoute, décalée d'UNE case par palier au-delà du premier (`frontierRadius = workRadius + max(0, paliers − 1)`, forme pure `frontierRadius` — le décalage est centralisé dans la fonction, pas dans le dessin).

**R-114 (rév. GP-CULTURE-EVENEMENTS — D1/D2/D3/D6, décision d'Erik du 13/09) · Personnages Illustres de culture — LE PALIER EST L'ÉVÉNEMENT.** Le canal culture lit la **culture de la CIVILISATION** = Σ des `city.cultureCumulee` du joueur (cumul EMPIRE, **jamais soustraite**) contre la **table T-27** (`culture.json` `greatPersonCultureThresholds` : 150, 267, 417… — ancres Erik) **indexée par le nombre de paliers DÉJÀ FRANCHIS** (`player.culturePaliers`, migration 22). Franchir un palier produit **DEUX conséquences atomiques, dans cet ordre** : **(a) +1 événement culturel** (`cultureMilestones += 1`, `reason: 'cultureLevel'`) **ET (b) l'apparition d'UN GP** — unité pacifique 0/0/2 (`greatPerson: true`), événement `GreatPersonSpawned`, posé dans la **ville à la plus haute culture cumulée** (D3 — tie-break cityId, R-81), sur sa case sinon première adjacente libre (perdu si aucune). **RÉV. CULTURE-FRONTIERES (14/09)** : `GreatPersonSpawned` porte un champ `canal` (culture/science/or/production/combat/artefact) — la SOURCE de chaque GP est explicite dans le journal (« GP de [canal] » ; Premier découvrir → « GP de technologie »), sans effet moteur ; **plusieurs paliers** peuvent être franchis dans la même résolution (boucle déterministe, par joueur en ordre R-81). **D2 — classe du GP** : TIRAGE UNIFORME SEEDÉ parmi les figures encore disponibles (RNG dédié `createRng(rngSeed ^ sel)`, miroir R-154 — chaque figure nommée ne sort qu'une fois, le pool s'épuise ; repli déterministe pool vide : rotation R-80 sur les 6 classes). **D4** : les GP d'autres voies (techs, or R-136, Confucius T-43, merveilles) incrémentent `greatPersonsObtained`/T-30 mais ne décalent JAMAIS le prochain palier. L'Anarchie gèle le canal (R-122). Les GP ne sont **jamais produits par les files** ; capturés en guerre ils sont détruits (R-43) sans jalon. La fusion D1 du 04/09 (`artiste_penseur`) reste en données ; l'ancien ciblage technologique R-127 est ABROGÉ. Voir §8.5.
**7l · C5 — Table canon des seuils culturels (révision T-27, Erik 05/09 ; index rév. GP-CULTURE-EVENEMENTS · D1/D4, 13/09)** : le « 20 ×2 » de 7f est **remplacé** par la **table canon d'Erik** — 1er GP à **150** points de culture, l'écart entre paliers croissant d'environ +33,33 à chaque GP obtenu (ancres d'Erik : 150, 267, 417, 600, 817, 1067, 1350, 1667, 2017, 2400, …, 15e = 4817, 20e = 8067). Implémentée en **table data-driven** (`culture.json` `greatPersonCultureThresholds`, générée par la formule « écart initial 117, croissance +33,33 par pas, arrondi » — contrôlée en tests contre les 12 valeurs d'ancrage ; extrapolation de la formule au-delà de la table). **Depuis D1 (13/09), la table est indexée par le nombre de PALIERS FRANCHIS (`culturePaliers`) et lue contre le cumul EMPIRE — plus par le nombre de GP obtenus.** Le seuil Leader (T-31) reste en l'état ; les seuils d'accumulateurs (T-30) ont été SUPPRIMÉS (RETRAIT-GP-ACCUMULATEURS, 14/09).
...
**R-116 · Nations Unies et victoire culturelle.** Merveille **unique à l'empire**, coût **`T-28`** (rév. 7l · C11 : **500**, `nations_unies.cost`), **verrouillée sous 20 jalons**, **non accélérable** (aucun GP ne peut hâter le chantier — la mécanique d'accélération n'existe pas encore). **Suspension** : si les jalons retombent sous 20 pendant la construction, la progression est **gelée** — les marteaux investis sont conservés 🔶 — jusqu'à repasser à 20. **GP-CULTURE-EVENEMENTS (D7, 13/09)** : la cause « vol de GP » a DISPARU (R-126 abrogée — le vol n'échange plus de jalons) ; la suspension ne peut plus être déclenchée que par la **perte d'une merveille** (capture de la ville hôte). Complétion → **`Victory(reason:'culture')`**. **Merveilles actives en 7f** : **Stonehenge** (Temples ×1,5), **Colosse de Rhodes** (commerce de la ville hôte ×2, appliqué au commerce brut AVANT la conversion R-90), **Jardins suspendus** (+50 % de population immédiat — arrondi au plus proche, citoyens auto-assignés). Une merveille **obsolète** (R-110/R-128) est retirée du menu de production ; les exemplaires bâtis **conservent** leur effet et leur jalon. La production d'une merveille **déjà bâtie QUELQUE PART** (révision 7k · R-129 — exclusivité mondiale) est refusée, comme une production dans un second chantier de l'empire ; une double complétion concurrente est un no-op documenté (la seconde ne compte pas — et, révision 7k · R-130, ses marteaux investis basculent en **récupération** si le compléteur est un rival).
...
- **vol de GP installé** (GP-CULTURE-EVENEMENTS · D7 — décision d'Erik du 13/09, rév. de la tranche 7g) : si la victime a au moins un GP **installé** (`settledGreatPersons` non vide) : le GP le plus récemment installé change de camp (réinstallé d'office dans la capitale du voleur — aucun `greatPersonsObtained` ne varie : **l'escalade T-27/T-30 est inchangée**) ; **AUCUN jalon n'est échangé** (R-126 abrogée — l'espion vole le GP et ses rendements, pas un point de victoire) ; l'Espion est **consommé** (`UnitDestroyed`, cause `mission`) ;
...
- ~~interaction R-116 par le vol~~ : SUPPRIMÉE (D7 — le vol ne fait plus varier les jalons ; la suspension ONU ne se déclenche plus que par la perte d'une merveille) ;
...
Base documentaire : la spécification d'Erik [`Gouvernements Civilization Revolution.md`](docs/recherche/Gouvernements Civilization Revolution.md) — **elle fait foi, valeurs exactes**. Toutes les données vivent dans **`governments.json`** (data-driven, calibrage sans code — même philosophie R-99/R-113) ; les thresholds GP restants dans **`culture.json`**.
...
**R-123 · GP restants — PARTIELLEMENT ABROGÉE (RETRAIT-GP-ACCUMULATEURS, décision d'Erik du 14/09).** Les GP proviennent UNIQUEMENT de : culture cumulée de l'empire (paliers T-27), certaines technologies, certaines merveilles, artefacts, **plus les deux canaux maintenus explicitement : le Leader (victoires de combat, T-31) et les paliers d'or (R-136)**. Les accumulateurs de rendement par ville (`city.gpAccumGold/Science/Prod/Food`, seuils T-30) sont SUPPRIMÉS — **aucun GP ne peut plus sortir d'un rendement** (science/or/production/nourriture), garde-fou testé (100 tours : seuls canaux observables culture/technologies/merveilles/artefacts/Leader/or). **Migration `schemaVersion` 22 → 23** : retrait des champs `gpAccum*` de chaque ville, idempotent, partie pré-23 reprise sans erreur. Historique (7h/7j) :
...
- **Magna Carta** : **Tribunal = +1 culture par citoyen et par tour** (tableau du doc d'Erik — **révision 7k** : le modèle 7h « +1/tour à plat 🔶 » est corrigé en « × population », écart doc/moteur listé à l'audit 7k ; data-driven `wonder.tribunalCulturePerCitizen`) ;
...
## 8.8 Personnages Illustres — classes canoniques, Consume & Settle — Phase 7j (ajouté le 03/09/2026)

Base : la spécification d'Erik [`Civilization Revolution _ Merveilles et Personnages.md`](docs/recherche/Civilization Revolution _ Merveilles et Personnages.md), section « Les Grands Personnages » (tableau Consume/Settle — elle fait foi). Sources croisées : CivFanatics Great People, StrategyWiki Famous People. **Décisions de tranche (pilotage 7j)** : le **ciblage technologique est une pondération DÉTERMINISTE** 🔶 (pas de nouveau RNG) ; les GP restent des **compteurs + entités de ville** (PAS d'unité mobile sur la carte — écart documenté avec le doc, modèle conservé, D4.4) ; les canaux **or (500 / 10 000 pièces) sont REPORTÉS 7l** (pas de trésorerie) ; le **flip culturel (Consume Artiste/Penseur) est REPORTÉ** (territoire en suspens).

**Classes canoniques (D1/D2 — R-114 rév., R-123 complétée).** Six classes, données `units.json` (`greatPerson: true`, pacifiques 0/0/2, jamais produits par les files) et `figures.json` (R-126bis) — renommages D2 : `scientifique`→**savant**, `mogul`→**explorateur** (Grand Explorateur / Industriel), `ingenieur`→**batisseur** ; fusion D1 : `artiste`+`penseur`→**artiste_penseur** ; **humanitaire** est NOUVEAU :

| Classe (id) | Canal d'obtention | Consume (ponctuel) | Settle (permanent) |
|---|---|---|---|
| Grand Artiste / Penseur (`artiste_penseur`) | culture T-27 (R-114) | conversion culturelle d'une cité ennemie — **INACTIF v1** (territoire en suspens) | **+50 % Culture** de la cité hôte |
| Grand Bâtisseur (`batisseur`) | ~~accumulateur production T-30~~ ABROGÉ (14/09) | **achève la production en cours** (unité, bâtiment ou merveille) | **−50 % marteaux** des futurs BÂTIMENTS de la cité |
| Grand Savant (`savant`) | ~~accumulateur science T-30~~ ABROGÉ (14/09) | **achève la recherche active** (complétion normale — Premier découvrir applicable) | **+50 % Science** de la cité hôte |
| Grand Explorateur / Industriel (`explorateur`) | ~~accumulateur or T-30~~ ABROGÉ (14/09) — reste le canal des paliers d'or R-136 | injection d'or (50/100/200/400 selon l'ère) — **INACTIF v1** (trésorerie, 7l) | **+50 % Or** de la cité hôte |
| Grand Humanitaire (`humanitaire`) | **canal CULTURE (révision 7k · C1 — veto d'Erik du 04/09 : il est produit comme les autres GP, ciblage technologique R-127 et rotation des 6 classes compris)** — l'accumulateur `city.gpAccumFood` (7j) est SUPPRIMÉ du moteur : d'abord DORMANT (compat saves), puis RETIRÉ DE L'ÉTAT par la migration 23 (14/09) | **+1 pop à TOUTES les cités** de l'empire | **+50 % croissance** (surplus alimentaire ×1,5) de la cité hôte |
| Grand Leader (`leader`) | victoires de combat T-31 (R-123) | **toutes les unités militaires → Vétéran** | **nouvelles unités de la cité vétérans** 🔶 (interprétation du « +3 XP / effet Caserne » — pas de modèle d'XP) |

**R-126 · Consume / Settle.** Chaque GP obtenu reste sur la carte **« en attente de choix »** ; le joueur le résout via le nouvel ordre **`GreatPersonAction { unitId, action: 'consume' | 'settle', cityId }`** (Phase C — `cityId` = ville amie sur sa case ou adjacente, comme R-115). Les effets sont EXACTEMENT ceux du tableau ci-dessus (le doc fait foi). Un ordre invalide (ville ennemie, trop loin, effet inactif/impossible) est **ignoré** — le GP reste en attente. `InstallPerson` (R-115) reste accepté comme **alias de `Settle`** (compat clients 7f/7h). Interprétations 🔶 documentées : multiplicateurs Settle **additifs** (+50 % par GP installé de la classe ; remise Bâtisseur ×0,5 par GP, multiplicative). Le bot applique un **choix déterministe simple 🔶 : toujours Settle**.

**C3 (révision 7k — veto d'Erik du 04/09) · Un seul GP d'un même type par ville.** Le **Settle d'une classe déjà installée** dans la cité cible est **refusé** (ordre ignoré, le GP reste en attente de choix) ; l'UI **désactive** le bouton avec un tooltip explicite. Les autres classes restent installables dans la même ville (les multiplicateurs Settle additifs 🔶 ne s'appliquent qu'entre classes différentes).
**C6 (révision 7l — décision d'Erik du 05/09, renforce C3) · Aucun effet cumulé au-delà d'une instance par classe.** Les multiplicateurs Settle **ne peuvent jamais se cumuler au-delà d'UNE instance par classe** : `settledGpMultiplier`/`settledGpCostFactor` plafonnent la classe à une instance (le modèle additif +50 % reste le code de base, il n'est simplement jamais multiplié — la double installation étant déjà refusée par C3, le plafond garantit aussi les chemins indirects comme la réinstallation d'un GP volé). Les tests « additifs » de 7j sont réécrits vers « une instance max ».

**Jalon culturel — ABROGÉ pour les GP (GP-CULTURE-EVENEMENTS · D6/D7 — décision d'Erik du 13/09, rév. du C2 du 04/09).** Les GP n'ont **AUCUN lien** avec `cultureMilestones` (R-126 abrogée) : obtention (canal culture, combat T-31, Premier découvrir), installation (Settle), consommation et **vol** (R-119) n'émettent **plus rien** (`reasons` `'obtain'`/`'install'`/`'gpStolen'` supprimées). Le compteur des 20 progresse par **paliers T-27 uniquement** (`reason: 'cultureLevel'` — le palier EST l'événement) **+ merveilles** (R-115/R-131). **La pénalité ICBM R-140 (−1 jalon 🔶) reste** et s'applique au total quelle que soit son origine. ~~L'escalade `T-30` reste alimentée par toute obtention de GP~~ (compteurs `greatPersonsByType` toujours incrémentés par obtention, mais plus lus par aucun seuil depuis le retrait des accumulateurs — 14/09) ; l'index T-27 (`culturePaliers`) n'est décalé que par la culture (D4).

**GP installés (D4.2/D4.3 — vol rév. D7, 13/09).** Champ **`city.settledGreatPersons: string[]`** — liste des classes installées dans la cité (source unique des multiplicateurs Settle R-126, du détail UI et du vol). **Seuls les installés émettent leurs bonus** (multiplicateurs appliqués en Phase C aux rendements de la cité hôte) ; un GP en attente de choix n'émet rien et **ne peut pas être volé** 🔶. Le vol d'installé (R-119) retire le GP le plus récemment installé de la ville cible et le réinstalle d'office dans la **capitale du voleur** (sinon première ville — le bonus Settle change de camp) ; **aucun jalon n'est échangé** (D7), escalade inchangée.

**R-126bis · figures.json.** Table data-driven des **figures historiques** par classe avec leur tech associée (libellés FR du doc — Ésope/Écriture, Homère/Alphabet, Platon/Poterie, J.S. Bach/Religion, Chéops/Maçonnerie, Léonard de Vinci/Invention, James Watt/Machine à vapeur, Thomas Becket/Féodalité, Frederick Douglass/Chemin de fer, Marco Polo/Monnaie, Christophe Colomb/Banque, Roald Amundsen/Combustion, Agamemnon/Travail du fer, Roi David/Monarchie, Archimède/Ingénierie, Confucius/Alphabétisation (`litteratie`), Albert Einstein/Théorie atomique). **Toutes les techs du tableau existent dans notre arbre** — aucune tech manquante signalée (D5.1).

**R-127 · Ciblage technologique — ABROGÉ (GP-CULTURE-EVENEMENTS · D2, décision d'Erik du 13/09).** La classe du GP du canal culture est un **tirage uniforme SEEDÉ** parmi les figures encore disponibles (`greatPersonClassTire`, RNG dédié dérivé du seed — miroir R-154 ; chaque figure ne sort qu'une fois ; repli pool vide : **rotation R-80** sur les 6 classes, index = `greatPersonsObtained`). **Aucune influence du joueur** (ni tech en cours, ni choix). Les autres canaux déterminent leur classe par nature (combat, or explicite R-136) ; les canaux d'accumulateurs sont abrogés (14/09) ; les octrois hors canal culture (Amérique, Confucius T-43) restent en rotation pure.

**D5.1 · Premier découvrir accorde un GP (R-109 étendu).** La récompense `firstToDiscover.greatPerson` (Invention → Grand Bâtisseur, Monarchie → Grand Leader) est **ACTIVE** : le GP de la classe de la figure de la tech est posé sur la première ville (sinon adjacente libre), **sans jalon** (révision 7k · C2), événement `FirstDiscovered` (champ `greatPerson`). Les canaux or (500 / 10 000) restent **reportés 7l**.

**Migration `schemaVersion` 12 → 13** : SANS PERTE — (a) **renommage des types d'unités** GP (`artiste`/`penseur` → `artiste_penseur`, `scientifique`→`savant`, `mogul`→`explorateur`, `ingenieur`→`batisseur`) ; (b) **fusion des compteurs** `greatPersonsByType` (`penseur`+`artiste` → `artiste_penseur`, renommages identiques) — l'escalade T-27/T-30 reste exacte ; (c) champs ADDITIFS par ville : `gpAccumFood: 0` (canal Humanitaire), `settledGreatPersons: []` (R-126 — aucun GP installé dans les états migrés, l'installation n'était pas enregistrée). Idempotent.

## 8.9 Merveilles du Monde — règles canoniques & effets restants — Phase 7k (ajouté le 04/09/2026)

Base documentaire : la spécification d'Erik [`Civilization Revolution _ Merveilles et Personnages.md`](docs/recherche/Civilization Revolution _ Merveilles et Personnages.md), sections « Les Merveilles du Monde » (règles canoniques + catalogue) et « Distribution cartographique » — **elle fait foi**. Sources croisées : [StrategyWiki Wonders](https://strategywiki.org/wiki/Civilization_Revolution/Wonders). **Décisions de tranche (pilotage 7k)** : la Grande Bibliothèque implémente la **condition canonique** (2 rivaux — jamais déclenchée en 1v1, documenté) ; **Oxford tire au RNG seedé R-80** (le canon dit « aléatoire ») 🔶 ; **Hollywood est inactif v1** (territoire en suspens) ; le libellé « coût d'achat » du Complexe militaro-industriel concernera le rush-buy (7l) — production seule pour l'instant ; les merveilles Artefacts (Angkor Wat, Arche d'Alliance, École de Confucius, Atlantide) sont une **phase séparée** (génération procédurale) — hors 7k.

**R-128 · Obsolescence GLOBALE (M1 — révision R-110, doc : « une Merveille perd son effet spécial dès qu'une civilisation sur la carte — propriétaire ou concurrente — découvre la technologie d'obsolescence »).** `isWonderObsolete` est évaluée sur l'**UNION des technologies connues de toutes les civilisations** (`allKnownTechs(state)` — l'appelant moteur fournit l'union, l'UI la recalcule sur l'état filtré, qui expose les `techsUnlocked` de tous les joueurs : l'obsolescence est une information publique par construction). Conséquences : la merveille perd son **effet** pour son propriétaire, est **retirée du menu de production de tous** (R-87/R-116), mais **la culture générée** (R-113) et **la valeur de Jalon** (R-115) sont **conservées après obsolescence** (le compteur de jalons lit les merveilles même obsolètes — `city.wonders`, indépendant de l'obsolescence).

**R-129 · Exclusivité mondiale (M2 — doc : « une Merveille ne peut être construite qu'une seule fois par partie, toutes civilisations confondues ; seule la première à cumuler le nombre de marteaux requis valide l'édifice »).** La validation `wonderProductionIssue` refuse une merveille **déjà bâtie n'importe où** (toute ville, tout propriétaire — révision de l'unicité d'empire 7f, qui reste vraie par inclusion) ; le chantier concurrent chez le rival reste autorisé (la course est légitime). **Complétion simultanée au même tour (rév. 7l · C8)** : gagne le chantier avec le **plus de marteaux en surplus** (investi − coût — un déficit de surplus plus faible l'emporte : le chantier le plus avancé gagne) ; **le perdant récupère l'ENTIÈRETÉ de ses marteaux** (bascule R-130/C7 — réserve permanente) ; **égalité de surplus → `cityId` croissant (R-81)**. La merveille **survit à la capture** (R-115 — inchangé, ne pas toucher sauf bug).

**R-130 · Récupération des marteaux — RÉSERVE PERMANENTE (M3 — rév. 7l · C7, décision d'Erik du 05/09 ; révoque la fenêtre T-32).** Adaptation aux **tours simultanés** : à la résolution où un rival complète la merveille que je construisais, **toute ville** qui avait cette merveille en chantier voit sa production basculer en **réserve** : nouveau champ de ville **`pendingSalvage`** (marteaux conservés, production vidée), événement **`HammerSalvage { cityId, owner, amount, outcome: 'available' }`**. **Révision 7l (C7) — plus de DISSIPATION** : la réserve **subsiste jusqu'à épuisement** ; **à chaque fin de tour où une réserve existe, le joueur DOIT choisir un projet** (dialogue UI forcé, miroir « unités sans ordre » — interprétation : le dialogue apparaît tant que la réserve n'est pas engagée dans un projet). **Consommation de la réserve en Phase C** (ville dotée d'une réserve ET d'un projet) :
- **projet NON répétable** (bâtiment, merveille) : si réserve ≥ coût → **complété immédiatement** et le **surplus RESTE en réserve** (ex. 200 récupérés, bâtiment 80 → produit ce tour, 120 restent) ; si réserve < coût → la réserve est **versée dans la progression** (reserve → 0) et le projet accumule normalement tour par tour ;
- **projet RÉPÉTABLE (unité)** : produite **autant de fois que la réserve le permet** (`⌊réserve / coût⌋`, pose sur la case de ville sinon adjacente — en attente si aucune case) ; le reliquat (< coût) est versé dans la progression de l'unité suivante (accumulation normale).
L'événement `HammerSalvage outcome:'dissipated'` est **SUPPRIMÉ** (l'événement devient inutile — le littéral `'dissipated'` est retiré de l'union d'événements, choix documenté : aucune persistance du journal n'en dépend). Interprétations 🔶 conservées : la continuation forcée de combat (R-55/R-56-3) et les attaques barbares ne sont pas des « attaques initiées » — voir R-133 ; la capture de la ville **emporte la réserve** (les marteaux n'appartiennent pas au captreur — `pendingSalvage` remis à 0). **Interaction rush-buy (R-135)** : un projet payé depuis la réserve ne consomme pas la trésorerie (les marteaux récupérés sont des marteaux, pas de l'or).

**R-131 · Merveille = Jalon culturel (M4).** À l'achèvement, une merveille compte comme un **Jalon** (+1 `cultureMilestones`, `reason: 'wonderBuilt'` — déjà implémenté 7f, vérifié 7k) : le compteur de jalons et les **Nations Unies** (20 jalons, R-116) l'intègrent ; le jalon **survit à l'obsolescence** (R-128) et suit la capture (R-115).

**R-132 · Effets des merveilles restantes (data-driven — `wonders.json`, valeurs du doc, tableau fait foi).**

| Merveille | Champ(s) moteur | Effet implémenté | Note |
|---|---|---|---|
| **Grande Bibliothèque d'Alexandrie** | `twoRivalsTechGrant` | À chaque résolution, accorde **toute tech déjà découverte par au moins 2 rivaux** (compté sur les AUTRES joueurs ; octroi direct — ni `firstBy` ni récompense Premier découvrir, événement `TechResearched` documenté) | En 1v1 (un seul rival) : **jamais déclenchée** — condition canonique (≥ 2 rivaux) implémentée, inactivité documentée |
| **Théâtre de Shakespeare** | `cityCultureMult: 2` | **×2 la Culture totale** de la cité hôte (après Palais/Temples/Stonehenge, avant multiplicateur Settle Artiste/Penseur) | modèle Stonehenge généralisé 🔶 (multiplie le total, y compris le bonus empire perCity) |
| **Université d'Oxford** | `randomTechOnComplete` | À la complétion : octroie **une technologie avancée aléatoire** — **tirage seedé R-80** parmi les techs non débloquées de l'empire (table triée par id 🔶), événement `TechResearched` | amendement R-80 documenté : le RNG est consulté en Phase C (comme les huttes en Phase A) |
| **Cie des Indes Orientales** | `oceanCommerceBonus: 1` | **+1 Commerce sur chaque case d'EAU exploitée** par la ville hôte — **rév. 7l · C9 : côte (`eau`) ET océan (`ocean`) inclues** (le canon dit « maritime » — Erik : toutes les cases d'eau) (bonus par terrain travaillé, modèle R-66 — intégré à `tileYield`, auto-assignation comprise) | révision du « `ocean` seul 🔶 » de 7k |
| **Atelier de Léonard** | `upgradeObsoleteUnits` | À la complétion : **met à niveau gratuitement toutes les unités obsolètes** de l'empire, en suivant les remplacements R-111 (`upgradeTo` en chaîne jusqu'à une unité non obsolète pour le propriétaire — armées comprises, vétérans/PV conservés), événement `UnitsUpgraded` | **une fois, à la complétion** 🔶 (pas un effet continu — doc/StrategyWiki) |
| **Foire de Troyes** | `cityGoldMult: 2` | **×2 la production d'Or de la cité** — multiplie la part **or** de la conversion R-90 de la cité (round half up final) | s'applique avant le multiplicateur Settle Explorateur |
| **Complexe Militaro-Industriel** | `militaryCostMult: 0.8` | **−20 % le coût de production des unités militaires** de l'empire — **rév. 7l (R-135) : multiplie AUSSI le coût d'ACHAT (rush-buy)** des unités militaires | le « coût d'achat » du libellé data est implémenté en 7l |
| **Internet** | `empireGoldMult: 2` | **×2 la production d'Or dans tout l'empire** — même interprétation 🔶 que Foire de Troyes (part or de la conversion R-90, toutes villes) | cumul avec Troyes : **MULTIPLICATIF ×4 (rév. 7l · C10 — remplace la convention MAX de 7k 🔶)** |
| **Programme Apollo** | `allTechsOnComplete` | Accorde **instantanément l'ensemble des technologies** de l'arbre (octroi direct — ni `firstBy` ni récompenses ; conséquence canonique : obsolescence globale R-128 de toutes les merveilles concernées) | effet massif endgame, implémenté tel quel 🔶 — **7l · C12 : les 46 entrées de journal `TechResearched` sont CONSERVÉES** (arbitrage pilot validé par Erik : « le bruit ne me dérange pas » — rien à changer) |
| **Hollywood** | (aucun) | **INACTIF v1** — le territoire (conversion culturelle) est en suspens ; données présentes, `implemented: false` conservé | comme le flip Artiste (R-126) |
| **Grande Muraille** | `blocksEnemyAttacks: true` | **L'adversaire ne peut pas attaquer tes unités ni tes villes** tant qu'elle est debout — décision d'Erik du 04/09 **validée** ; portée empire ; vaut contre toute nation ennemie (barbares compris) | jusqu'à l'obsolescence M1/R-128 (Ingénierie — union des techs) ; les attaques sont **annulées avant consommation de PM** (ordre `Attack` : fizzle ; entrée de case R-42 : arrêt devant le défenseur, chemin conservé) — continuation forcée R-55/R-56-3 **non bloquée** (terminaison garantie 🔶) |

Hors périmètre 7k : **Projet Manhattan/ICBM** (7m), **Banque mondiale** (7l — activée §8.10), **SETI et merveilles DLC** (exclus v1).

## 8.10 Or & trésorerie — Phase 7l (ajouté le 05/09/2026)

Base documentaire : la spécification d'Erik [`Économie d'or Civ Revolution.md`](Économie%20d'or%20Civ%20Revolution.md) — **elle fait foi** (trésorerie, zéro entretien, rush-buy, paliers, Banque mondiale). **Compléments de calibrage canon : [`Civilization Revolution Rules Research.md`](docs/recherche/Civilization Revolution Rules Research.md) et [`Calibrage Mécaniques Civilization Revolution.md`](docs/recherche/Calibrage Mécaniques Civilization Revolution.md) (Erik, 06/09)** — facteur rush Industrielle ×4 (coûts console dataminés), sac de ville 25 %, classe GP or = Explorateur/Industriel. Données : **`economy.json`** (facteurs d'ère du rush, paliers, sac de ville, intérêts, injection Explorateur — calibrage sans code, même philosophie R-99/R-113). La conversion binaire par ville (R-90) est **confirmée fidèle au canon** (focus Or/Science binaire par ville) — inchangée ; l'or qu'elle produit **alimente désormais la trésorerie** au lieu de s'évaporer.

**R-134 · Trésorerie d'empire.** Champ joueur **`treasury`** (migration 14→15 — l'ancien `player.gold`, jamais dépensé, devient la trésorerie initiale : **report de valeur, zéro perte** ; le champ `gold` est supprimé). Créditée en **fin de tour (Phase C)** par la somme des villes **focus Or** (gains de conversion R-90 + bonus empire Premier découvrir). **ZÉRO ENTRETIEN** : aucun coût d'or pour les bâtiments, les unités ou la population (règle explicite du canon — test négatif).
- **Sources d'or exogènes** (toutes créditées à `treasury`) :
  - **Surplus de recherche** : l'excédent de fioles au moment où une tech est découverte est converti **1:1 en or** — **révision de R-85/R-86 (« le débordement est reporté »)** : le canon convertit l'excédent, il ne le conserve pas (écart appliqué et signalé 🔶). La réserve `scienceStored` reste ce qu'elle était : la science accumulée SANS tech choisie (elle n'est pas un « surplus » — elle se verse sur la première tech choisie, inchangé).
  - ~~**Camp barbare détruit = +50 or**~~ **SUPPRIMÉ (rév. BARBARES-PILES 15/09)** — la capture d'un camp tire désormais la récompense hutte seedée (R-96 ; événement `VillageLooted`, plus de `BootyGold` côté village).
  - **Sac de ville** : capturer une ville ennemie crédite le captreur d'une **part de la trésorerie du perdant** — `economy.json` `cityCapturePlunderPct` **0.25** (calibrage canon — rapport « Calibrage Mécaniques Civilization Revolution » §2, Erik 06/09 : protège la Victoire économique contre le snowballing ; arrondi au plus proche, minimum 0 ; la capitale terminant la partie, le sac s'applique aux villes secondaires) ; champ `plunder` ajouté à l'événement `CityCaptured`.
  - **Ressources Gemmes/Or** : **correction du canal 7c (D3)** — le trésorerie existant, le canon est appliqué : Gemmes **+2 or/tour**, Or **+3 or/tour** (condition Monnaie — `revealedByTech` inchangée), crédités **directement à la trésorerie** quand la case est travaillée (nouveau champ `directGold` de `resources.json` ; le bonus commerce de ces deux ressources est retiré). Les 20 autres ressources gardent le canal N/P/C (R-93).
  - **Caravane** : l'unité existe en données (`units.json` `caravane`, `implemented: false`) — **REPORTÉ** (paiement forfaitaire à l'arrivée, hôte ~40 % 🔶) avec le reste de l'espionnage/commerce, consigné au rapport.
  - **Vol d'or par espion** : **reporté 7m** (avec contre-espionnage), consigné.
- **Intérêts 2 %** (hook 7n) : la mécanique existe — à la fin de Phase C, un joueur doté du trait d'intérêts crédite `treasury += round(treasury × 0.02)` (`economy.json` `interestRatePct` 🔶 0.02). **Désactivée sans trait** (aucun système de civilisations avant 7n — hook `playerHasTrait` toujours false, test de la désactivation + test unitaire du calcul pur).
- **Paliers économiques** : voir R-136.

**R-135 · Rush-buy (achat instantané).** Coût en or = **marteaux restants × facteur d'ère** (strictement linéaire ; 0 marteau investi = coût total × facteur) : `coût = round(max(0, coût_item − progression) × facteur(ère) × réductions)` (round half up). L'**ère** est celle de l'empire (compage R-147). Facteurs **data-driven** (`economy.json` `eraRushFactors`) — **calibrage canon (rapport Calibrage §1, Erik 06/09, coûts console dataminés)** : **Antique ×2** (Guerrier 10 → ~20-30 ; Pyramides 150 → 300-450), **Médiévale ×3** (Marché 60 → 180 ; Banque 120 → 360), **Industrielle ×4** (Marché 60 → 240 ; Banque 120 → 480), **Moderne ×8** (Bibliothèque 40 → 320 ; Banque 120 → 960) — progression canon ×2/×3/×4/×8.
- **Réductions cumulables (multiplicatives)** : **Complexe militaro-industriel −20 %** sur les unités militaires (multiplie aussi le coût d'ACHAT — R-132) ; **hook 7n** : futur trait civilisationnel « rush à moitié prix » (×0,5), hook `rushDiscountOf` inactif sans civils.
- **Éligibilité** : unités (militaires **ET** civiles — Colon compris), bâtiments, merveilles classiques, composants du Vaisseau spatial. **INTERDITS : Banque mondiale et Nations Unies** (test explicite). **Un seul rush par ville et par tour** (le serveur remplace l'ordre de même sujet — un seul `RushBuy` par ville peut exister). **Ville fondée ce tour : achat immédiat autorisé** (test).
- **Ordre `RushBuy { cityId }`** — nouvelle forme validée par `orderShapeError` **EN PREMIER** (leçon 7f) ; re-validé par le moteur à la résolution : ville possédée, production en cours, item éligible, trésorerie suffisante — sinon ignoré (aucun débit). L'achat complète l'item **immédiatement** (Phase C, avant l'économie : unité posée sur la case de ville sinon adjacente — coûts de population du Colon R-112 applicables ; bâtiment posé avec remplacement R-111 ; merveille via la complétion canonique R-129/R-130/R-131), débite la trésorerie, émet `RushBuy { cityId, owner, item, cost }` puis les événements de complétion usuels.
- **Hammer banking proscrit (canon)** : basculer la production d'une merveille vers **ONU ou Banque mondiale** réinitialise les marteaux accumulés à **0** (la conservation R-62 ne s'applique pas à cette transition — test). Pour les autres basculements, la conservation R-62 reste (le canon ne documente que le cas victoire — écart éventuel signalé 🔶).

**R-136 · Paliers économiques (ladder canon).** Chaque palier est accordé **une seule fois**, **dans l'ordre des seuils**, quand la trésorerie **franche** le seuil (plusieurs paliers peuvent tomber le même tour — grand saut compris ; les paliers sont suivis par un compteur joueur `economyMilestonesClaimed`, migration 14→15) :

| Seuil | Récompense |
|---|---|
| 100 | **Colon gratuit** à la capitale |
| 250 | **Tech économique gratuite** (Monnaie, sinon Bancaire — la première non débloquée) |
| 500 | **Grand Personnage gratuit** (canaux GP par or du doc GP — 500 / 10 000 confirmés) |
...
| 20 000 | **Débloque la Banque mondiale** (condition dynamique R-137) |

Table **data-driven** (`economy.json` `milestones`). Les bâtiments gratuits suivent R-66/R-111 (déjà possédé = saute ; remplacement applicable) ; les citoyens des villes grossies sont auto-assignés (R-60). Le GP des canaux or est posé à la **capitale** (sinon première ville — canon « apparaît aux abords de la capitale ») et **sans jalon** (miroir C2 — seuls les GP du canal culture comptent). **Révision Calibrage canon (Erik 06/09)** : la classe du GP or est **EXPLICITE** — **Grand Explorateur/Industriel** (classe `explorateur`, paliers 500 et 10 000 or → cette classe) et non le ciblage technologique R-127 ; si la liste de figures de la classe est épuisée, rotation de secours **Bâtisseur puis Savant**. Chaque palier émet `EconomyMilestone { player, threshold, reward, label }` (libellés FR) + les événements d'effet concrets (`UnitProduced`, `TechResearched`, `GreatPersonSpawned`, `BuildingCompleted`, `PopulationGrew`).

**R-137 · Banque mondiale & victoire économique.** Merveille **500 marteaux** (donnée), champ `economicVictory: true` + `treasuryRequired: 20000` 🔶 (donnée `wonders.json` — alignée sur le dernier palier R-136).
- **Condition DYNAMIQUE** : verrouillée tant que `treasury < 20000` — elle **apparaît/disparaît du menu** de production (validation `wonderProductionIssue` étendue : contexte `treasury`) ; si le joueur repasse sous 20 000 **pendant le chantier**, la progression est **gelée** (miroir ONU R-116 — marteaux conservés) jusqu'à repasser au-dessus.
- **L'or n'est PAS débité** : les 20 000 sont une **condition**, pas un prix (test explicite — la trésorerie est intacte à la complétion).
- **Non achetable** (R-135 — interdit au rush-buy).
- Complétion → **`Victory(reason:'economique')`** — **quatrième victoire** : écran de victoire, motif méta serveur, bot, stats.

**R-133 · Audit des 8 merveilles actives contre le tableau canonique (modèle 7i — écarts listés/corrigés).**
- **Stonehenge** (+50 % culture des Temples) : conforme (templeCultureMult 1,5, obs. Littératie).
- **Grande Pyramide** (tous les gouvernements sans recherche) : conforme (allGovernments + `governmentIssue` 7h, obs. Monarchie).
- **Colosse** (×2 Science **et** Or de la cité) : conforme en substance — `commerceMult` 2 s'applique au commerce brut AVANT conversion R-90 : la conversion binaire envoie 100 % du commerce vers l'or OU la science, donc ×2 vaut pour les deux (obs. Invention).
- **Oracle** (avertissement si une attaque risque d'échouer) : conforme (battleForeknowledge — UI `UnitPanel` : probabilités exactes + vainqueur attendu, obs. Religion).
- **Jardins suspendus** (+50 % pop de la cité) : conforme (populationGainPct 0,5 immédiat, arrondi au plus proche ; pas d'obsolescence — doc).
- **Château Himeji** (+1 attaque empire) : conforme (attackBonusEmpire 1, obs. Communisme).
...
Base documentaire : la spécification d'Erik [`Nuclear and spy Game Mechanics Research.md`](docs/recherche/Nuclear and spy Game Mechanics Research.md) — **elle fait foi, SAUF les décisions d'Erik du 05/09 (Bloc 0 ci-dessous), qui ont préséance**. Données : **`espionnage.json`** (pénalité culturelle, part d'or volée, matrice de duel — calibrage sans code 🔶, même philosophie R-99/R-113) ; `units.json` (`icbm` audité 0/0/40 `strategic`, `espion` audité 0/0/2), `buildings.json` (`sdi` — audité conforme), `wonders.json` (`projet_manhattan` — audité, activé). Décisions de tranche (pilotage 7m) : la résolution ICBM est **unifiée data-driven** (une seule règle capitale/ville — réversible à l'acceptation, voir C13) ; le RNG est consulté **en Phase C** pour les duels d'espions et la sélection des bâtiments détruits (amendement R-80 documenté, miroir Oxford R-132) ; la Grande Muraille ne bloque pas l'ICBM 🔶 ; un espion capturé en rase campagne ne rapporte pas de butin 🔶 ; la vision de l'ICBM est 2 🔶 (canon muet).

### Bloc 0 — Décisions d'Erik du 06/09 (préséance — révisions 7n des décisions du 05/09)

**C15 · La distinction canon de l'ICBM est RÉTABLIE (révision de C13).** La règle « ville survit, pop 2, moitié des bâtiments, merveilles préservées » s'applique **à la CAPITALE seulement**. Une **ville ordinaire est RASÉE** (canon) : ville **effacée de la carte** (population zéro), bâtiments **et merveilles** détruits (jalon perdu par merveille — R-115, miroir du rasement barbare R-97), garnison anéantie (C13.4 conservé) et **cratère** — la case devient le terrain `cratere` **stérile** (rendements nuls, ressource effacée 🔶) et **non fondable** 🔶 (défaut : cratère permanent ; alternative : réutilisable après N tours — le canon est muet). Le rayon 1 (6 tuiles) annihile toutes les unités dans les deux cas (C13.4 conservé). Tests C13 réécrits (capitale + ville ordinaire).

**C16 · Moitié des bâtiments : arrondi vers le HAUT** (⌈n/2⌉ — 5 bâtiments → 3 détruits) ; sélection seedée R-80, Palais exclu (inchangé).

**C17 · La Grande Muraille bloque le missile nucléaire (révision R-140).** Tant qu'elle est debout (non obsolète — obsolescence GLOBALE R-128, union des techs), toute frappe visant une ville du propriétaire de la Muraille est **annulée** — missile **consommé**, **aucun dégât** (`NukeLaunched` outcome `blocked` — miroir SDI R-141) ; **portée empire** (miroir de son effet d'attaque R-132). 🔶 la Muraille ne bloque pas les tirs sur les **cases adjacentes** (l'exploit R-141 reste possible) ; une frappe `blocked` n'est pas une détonation : pas de pénalité culturelle, `nukesLaunched` non incrémenté.

**C18 · Destruction de bâtiment par espion : choix, coût et risque (révision R-143.4).** Le tireur choisit le bâtiment **avant** l'action (`buildingId` déjà porté par l'ordre) ; **le coût et le risque croissent avec la valeur de production** du bâtiment (« plus facile de détruire une Bibliothèque qu'une Université »). **Défauts data-driven 🔶 (`espionnage.json`)** : coût en or = `round(marteaux × 0,5)` **débité au lancement, non remboursé** (échec compris) ; réussite = `clamp(0,9 − marteaux/500 ; 0,4 ; 0,9)` (RNG seedé R-80, consulté après le duel R-144) ; **échec du tirage = espion perdu + or perdus** (défaut 🔶) ; **trésorerie insuffisante** = action sans effet (aucun débit, espion survit 🔶) ; merveilles non ciblables, Palais exclu (inchangé). Le duel R-144 précède le débit : un espion perdant le duel ne paie pas. Veto Erik possible sur les deux formules.

### Bloc 0 initial — Décisions d'Erik du 05/09 (préséance sur le rapport ; C13/C16/R-140 révisés ci-dessus)

**C13 · Résolution ICBM (révision du canon — vaut pour la CAPITALE, voir C15).** Toute ville ciblée (capitale ou non) **survit** — pas de destruction totale, pas de cratère :
...
## 8.12 Civilisations & traits — Phase 7n (ajouté le 06/09/2026)

Base documentaire : la spécification d'Erik [`Guide Civilisations Civilization Revolution.md`](docs/recherche/Guide Civilisations Civilization Revolution.md) — **elle fait foi** (16 civilisations : avantage de départ, 4 bonus d'ère **CUMULATIFS**, unités uniques). **Complément de calibrage canon : [`Calibrage Mécaniques Civilization Revolution.md`](docs/recherche/Calibrage Mécaniques Civilization Revolution.md) (Erik, 06/09)**  Aztèques +25 or dataminé, Russie rayon 5 confirmé, Grèce/Rome ×0,75 confirmé, Zoulous = Aqueduc passif (seuils ÷2), Égypte = tirage seedé sans choix, techs gratuites comptées dans les seuils d'ère 5/14/24 (confirmés). Sources croisées : [CivFanatics — Civilizations CivRev](https://civfanatics.com/civrev/civilopedia/civilizations/) (bonus conformes — écarts de libellé « 85mm Gun » vs « Canon 88mm » et montagnes « +2 food » vs doc « +2 Production » signalés au rapport ; **le doc fait foi**), [Fandom — List of units](https://civilization.fandom.com/wiki/List_of_units_in_CivRev) (uniques = stats de l'unité remplacée ; les deltas du doc — Impi +1 M, Longbow +1 D, Samouraï +1 A, Obusier +2 A — sont portés par les traits d'ère). Données : **`civilizations.json`** (civs + traits + params 🔶) et **`eras.json`** (seuils d'ère 🔶) — calibrage sans code, même philosophie R-99/R-113.

**Décisions de tranche (pilotage 7n, veto Erik possible)** : l'ère passe au **COMPAGE de technologies** (défaut canon — voir R-147) ; le bonus d'ère « Ancienne » est actif **dès le début de partie** (le joueur commence dans l'ère Antique — une civ en ère Moderne a donc son avantage de départ **et** ses 4 bonus d'ère actifs simultanément, doc §Cumulativité) ; choix de civ **à la création pour l'hôte, au join pour l'invité**, stocké dans `meta.players` ; les **traits non mappables sont documentés INACTIFS** (routes — pas d'ouvriers ; caravanes — reportées 7l ; élite automatique & Loyauté — pas de système d'XP/promotions) ; l'art des unités uniques est un **alias teinté** du sprite de l'unité remplacée 🔶 (24 planches dédiées reportées — `unite_<id>.png` prioritaire s'il est fourni) ; **deux joueurs peuvent choisir la même civ** 🔶 (aucune interdiction canon tranchée) mais pas la même **Merveille Antique** d'Égypte (exclusivité mondiale R-129).

**R-145 · Structure des civilisations (civilizations.json).** Chaque civ porte : `id`, `name`, `leader`, `start[]` (avantage de départ), `eras{ancienne, medievale, industrielle, moderne}[]` (bonus d'ère **cumulatifs — jamais éteints**), `uniqueUnits[]` (ids R-148). Chaque trait est une entrée `{key, label, ...params, inactif?}` d'un **catalogue fermé de clés typées** (R-146). Une civ inconnue ou `neutre` = aucun trait (parties migrées, fixtures). La civ est **publique** (canon : l'adversaire voit la civ adverse — diffusée telle quelle par l'état filtré).

**R-146 · Catalogue de traits.** Clés moteur actives : `interets` (2 % trésorerie R-134), `rushHalfPrice` (scope `unit` — Amérique Médiévale : rush des **UNITÉS** ×0,5 R-135), `terrainBonus` (par terrain — plaine/désert/forêt/colline/montagne/eau/océan, s'applique à la récolte R-66/R-93), `buildingProductionMult` (Amérique Moderne : Usine ×3), `gpGratuit` (Amérique départ — classe R-127, sans jalon), `orDepart` (+25 Aztèques — canon dataminé, montant fixe au tour 1), `soinVictoire` (PV max après victoire de combat — Aztèques), `templeScience` (Aztèques : Temple +3 science/ville), `empireGoldMult` (Aztèques/Espagne/Zoulous — multiplicatif avec Internet/Troyes, miroir C10), `commerceCaptures` (Mongols : +50 % commerce des villes **capturées** — champ ville `wasCaptured`, définitif), `villagesVilles` (Mongols : hutte ouverte → **ville pop 1** au lieu de la récompense 🔶 — si fondation illégale T-09, récompense normale), `popFondation` (Chine +1, Rome Moderne +1 → pop 6), `immuniteAnarchie` (Chine Moderne/Inde Antique/Japon Industrielle — SetGovernment sans Anarchie), `coutUniteMoitie` (Inde Colons, Russie Fusiliers/Espions), `coutBuildingMoitie` (Bibliothèques Chine/Grèce, Casernes Allemagne, Tribunaux Inde), `coutMerveilleMoitie` (Rome Médiévale), `techGratuite` + `gouvernementGratuit` (départs), `batimentDepart` (France Cathédrale, Grèce Tribunal — prérequis R-111 non exigés au setup), `merveilleAntiqueDepart` (Égypte — Merveille Antique **tirée au RNG seedé** de génération parmi les 6 canoniques, SANS choix du joueur — Calibrage 06/09), `unitAttack`/`unitDefense`/`unitMovement` (par type d'unité — Arabie cavaliers/chevaliers, Angleterre archers, Mongols cavalerie, Zoulous guerriers, France canons+obusier, Égypte/France fusiliers ; les ids d'uniques y figurent quand le doc les vise), `uniteVeteran` (Allemagne : Guerriers produits vétérans), `navalAttack` (Angleterre/Espagne : +1 attaque navale), `soutienNavalDouble` (Angleterre Moderne : soutien R-118 ×2), `carteRevelee` (Russie : rayon 5 au départ — canon : ~2-3 cases au-delà de la vision du Colon, confirmé), `toutesRessources` (Inde — R-93 court-circuité), `overrun` (Zoulous : ratio 4 — R-149), `croissanceSeuilDivise` (Zoulous Médiévale — mécanique canon **Aqueduc passif**, dataminé : les seuils de croissance de `growth.json` sont **divisés par deux** pour toutes les villes de l'empire, sans le bâtiment ; ni un multiplicateur de nourriture ni de vitesse ; se combine multiplicativement avec la réduction de l'Aqueduc — remplace l'ancien trait `croissanceAcceleree` 0.33), `tresorsDouble` (Espagne : or des huttes ×2 🔶 — artefacts : phase suivante), `gpFrequents` (Grèce/Rome : seuils GP ×0,75 — canon confirmé, fourchette 25-50 %, borne basse retenue — canal culture T-27 seul depuis le retrait des accumulateurs T-30 (14/09)). Traits **INACTIFS** (données portées, UI grisée, moteur ignore) : `routesMoitie`, `caravanesPlus50`, `eliteAuto`, `loyaute`.

**R-147 · Ère par COMPAGE de technologies (T-36 🔶 : Médiévale 5, Industrielle 14, Moderne 24 — eras.json, indifférent à la branche).** REMPLACE la définition 7i « tech la plus avancée » (`techEraOf`) — utilisée PARTOUT : pop de fondation (R-64/D3), facteurs de rush (R-135), injection Explorateur, activation des bonus de civ, Overrun. L'ère est **persistée** (`player.era`, migration 17) et la transition est appliquée **AU TOUR SUIVANT** (fin de Phase C) : une tech complétée pendant la résolution N change l'ère pour N+1 ; la pop de fondation et le rush de la résolution N lisent l'ère ancienne. Événement **`EraChanged`** (public — l'ère est une information publique). Au franchissement d'un palier, les **techs gratuites** du palier de la civ (Arabie Mathématiques, Chine Alphabétisation, Égypte Irrigation, Inde Religion, Mongols Communisme, France Poterie, Démocratie de la Grèce au départ…) sont accordées par **octroi direct** (grantTech — ni `firstBy` ni Premier découvrir) et le comptage est réévalué (cascade déterministe). Les techs de départ **comptent** dans le compage (canon), comme TOUTE technologie obtenue gratuitement — techs de civ, paliers d'ère, paliers économiques, artefacts, huttes (canon confirmé par le rapport Calibrage « Inclusion des technologies gratuites : OUI » — test dédié).

**R-148 · Unités uniques (~24 — pattern R-111).** Données `units.json` : `uniqueTo: <civId>` + `replaces: <unitId>` + stats **de l'unité remplacée** (canon Fandom — les deltas du doc sont portés par les traits d'ère R-146). L'unique est **proposé à la production dès que sa tech est débloquée** (la sienne = celle de l'unité remplacée) et **retire l'unité standard du menu** de production de sa civ (`canSetProduction`/`isProducible` refusent le standard remplacé, l'unique, et tout unique pour une autre civ) ; le remplacement s'applique aussi aux **unités de départ de la carte** et aux **unités gratuites** (huttes, Premier découvrir, paliers, réserve C7). L'unique partage l'**obsolescence** de l'unité remplacée (l'Impi devient obsolète avec le Guerrier — R-110). Les uniques aériens (Mustang, Spitfire, ME109, Zero, Flying Fortress, Lancaster, Heinkel, Val) restent en **données seules** (`implemented: false` — 7h+). Art : alias teinté 🔶 (voir décisions de tranche).

**R-149 · Application moteur des traits (source unique `civilizations.ts`).** Tous les helpers sont **purs et déterministes** (R-81) : bonus de terrain dans `tileYield` (récolte, ressource Inde comprise), coûts effectifs via `productionItemCostOf` (rush compris), pop de fondation `foundingPopForEra + popFondation`, immunité Anarchie dans `applySetGovernment`, **Overrun** ajouté au combat §7 : avant l'échange, si `S_att_base ≥ ratio × S_def` (base canon **6** pour tous ; Zoulous **4** 🔶) et `S_def > 0` 🔶 (ratio indéfini contre une défense nulle), le défenseur est **détruit instantanément** — pas de round R-51, vétérans/jalons/soin applicables ; mêlée uniquement (jamais pour un attaquant à distance R-59), et le **soutien naval ne compte pas** dans le ratio (interprétation 🔶 : le bombardement n'aide pas à « écraser » — comportement R-118 préservé). Le soin `soinVictoire` rend les PV max à tout coup fatal (recordCombatVictory). Les bonus d'attaque/défense/mouvement s'appliquent dans `S_att`/`S_def` (§7.4, indépendants de l'Anarchie) et à la régénération PM (R-72) ainsi qu'à la création d'unités.

**R-150 · Avantages de départ (setup déterministe).** `createInitialState(map, seed, civSetup?)` applique, dans l'ordre : techs de départ (octroi direct, sans événement) → gouvernement de départ (sans Anarchie) → bâtiments gratuits de la capitale → **Merveille Antique de l'Égypte (révision Calibrage canon — Erik 06/09 : TIRAGE SEEDÉ** au RNG **dédié** dérivé du seed de GÉNÉRATION — pas de résolution — parmi les **6 Merveilles Antiques canon** : Colosse, Grande Pyramide, Grande Muraille, Jardins Suspendus, Oracle, Stonehenge (`egypteWonderChoices`) ; construite gratuitement dans la capitale à la fondation, **sans choix du joueur** — plus aucun champ `wonderId` au lobby/setup/UI ; même seed ⇒ même merveille) → or de départ → GP gratuit (Amérique, classe R-127 rotation — sans jalon) → révélation de carte (Russie) → **remplacement des unités de départ** par les uniques disponibles (R-148) → vétérans de trait (Allemagne). Le serveur construit `civSetup` depuis `meta.players` (choix de CIV au lobby — hôte à la création, invité au join) ; la même graine + les mêmes civs → le même état (idempotent, crash-recovery §3.5).
...
| T-15 | `growthBase` | **OBSOLÈTE (7i, table rév. 7l · C4)** — remplacé par la table `growth.json` (`growthThresholds` indexée par population cible, **linéaire 10 × n : seuil(cible) = 10 × (cible − 1)** — décision d'Erik du 05/09 ; plafond 31 ; fondateur par ère `founderPopByEra` 2/3/4/5 🔶 ; tranches R-60bis 🔶) |
...
| T-20 | ~~`villageDestructionGold`~~ | **SUPPRIMÉ** (BARBARES-PILES 15/09 — récompense hutte seedée à la place) |
| T-21 | ~~`villageHP`~~ | **SUPPRIMÉ** (BARBARES-PILES 15/09 — camps sans PV) |
...
| T-27 | `greatPersonThresholdBase` | **OBSOLÈTE (rév. 7l · C5 — décision d'Erik du 05/09)** — remplacé par la **table canon** `culture.json` `greatPersonCultureThresholds` (150, +33,33 croissant : 150, 267, 417, 600, …, 15e = 4817, 20e = 8067 — R-114) |
| T-28 | `unitedNationsCost` | **500 (rév. 7l · C11 — décision du 04/09, doc)** (R-116 — `wonders.json` `nations_unies.cost` 300 → 500) |
...
| T-38 | `artefactCount` | 4, borné **4–5** (R-151, Phase 7o — `artefacts.json` ; tirage sans remise, Atlantide toujours ; calibrage canon : 4-5 par carte) |
| T-39 | `artefactPlacement` | îles ≤ 5 cases, ≥ **8** des départs, continent ≤ 2, espacement 4 🔶 (R-152 — `artefacts.json`) |
...
| T-42 | `templiersUnitByEra` | chevalier / chevalier / canon / char d'assaut 🔶 (R-154 — `artefacts.json`) ; l'unité arrive TOUJOURS Vétérane (5 XP — Calibrage 06/09) |
...
## 11bis. Génération procédurale des cartes — Phase 6b (ajouté le 02/09/2026, transcription de [`HANDOFF-PHASE6B.md`](docs/historique/handoffs/HANDOFF-PHASE6B.md), base documentaire : [`Génération Procédurale Cartes Civilization.pdf`](Génération%20Procédurale%20Cartes%20Civilization.pdf))
...
- **R-103 · Fertilité & normalisation** : score = Σ anneaux 1/2/3 pondérés 🔶 [1.0, 0.6, 0.3] × (nourriture ×2 + production ×1.5 + commerce ×1), pénalité 🔶 par montagne (la montagne reste travaillable R-60) ; ressources incluses. Site de capitale = meilleur candidat praticable (≥ 6 des bords, ≥ T-09 de l'axe, distance au site image ≥ 12, ≥ 1 voisin libre pour le Guerrier). **Anneau de départ équilibré (Phase 6c, demande d'Erik) — REMPLACÉ par R-157 (SPAWN-START, 05/09)** : le filtrage des candidats par minimums d'anneau (`startMinRingPrairie`/`startMinRingForest`, settings supprimés) laisse place au **re-paint forcé** du voisinage du site choisi — voir R-157. **Démarrage COLON + Guerrier sans capitale (Phase 6c, décision d'Erik)** : les cartes procédurales portent `start: "colon"` — le Colon OCCUPE le site réservé (fondation à coût nul via FoundCity R-64, ou déplacement assumé), le Guerrier est adjacent, **aucune ville à l'initialisation** ; la première ville fondée devient capitale (R-64) et la victoire par domination redevient possible après fondation. Les cartes préfabriquées restent en démarrage `capital` (défaut du champ optionnel `MapData.start`). **Normalisation** (PDF §NormalizeStartLocation) : si le site est sous le seuil 🔶 (moyenne des N=5 meilleurs sites × 0.8), injecter bétail/blé (données R-91) — depuis **R-157 (SPAWN-START)** : **anneau 3 uniquement** (les anneaux 1-2 du site sont réservés : rayon sans ressource) — jusqu'au seuil, échec explicite sinon ; la garantie de couverture R-108 exclut aussi le rayon des capitales (R-157). Checksum d'équité (delta P1−P2 = 0 par miroir, score ≥ seuil, **composition de voisinage identique — R-157**) consigné dans `meta.progen` → dump admin.
...
- **R-157 · Garantie de voisinage du départ (SPAWN-START — demande d'Erik, 05/09)** : le Colon de départ est placé de sorte que **1)** les **6 cases adjacentes** à son site comptent **exactement 🔶 2 forêts et 🔶 1 case d'eau, au moins 🔶 2 prairies**, la **6e case libre** restant tout terrain productif non-montagne (si elle est impropre — montagne/eau/cratère — elle est repeinte en prairie 🔶 ; une voisine déjà conforme peut s'ajouter aux minimums) ; et **2)** **AUCUNE ressource** (R-91) dans les 6 adjacentes **ni à distance 2** du spawn (rayon 🔶 `spawnPurgeRadius` = 2 — anneaux 1 et 2 purgés ; les **artefacts** (7o) ne sont PAS concernés : entités disputées, hors R-91 ; huttes/barbares : règles 7d inchangées). **Placement d'abord, carte ensuite** 🔶 : le site est choisi selon les critères R-103, puis son voisinage est **FORCÉ par re-paint** (`forceSpawnNeighborhood` — les terrains déjà conformes sont préservés en priorité, ordre déterministe (q, r) R-81) puis le rayon est **purgé** (`purgeResourcesNear`) — un filtrage des candidats ne garantit pas qu'un tel voisinage existe naturellement. **Miroir (R-102)** : le re-paint de la demi-carte est reflété par la rotation 180° → les DEUX spawns reçoivent le même traitement, la composition des deux voisinages est identique (**checksum d'équité étendu** — assertion fail-loud + consigne `report.spawn` au dump admin/labo) ; la garantie de couverture R-108 et les ressources marines ne peuvent plus poser dans le rayon (zone d'exclusion étendue + filtre final). **Cartes préfabriquées** : auditées et ajustées à la même garantie (rapport SPAWN-START). Tests statistiques : 100 % des seeds testés conformes (`progen-spawn-start.test.ts`).
...
| ~~X-2~~ | ~~Halte (R-42)~~ | **ABROGÉE (décision d'Erik du 18/09)** — la découverte d'un ennemi n'arrête PLUS les chemins : une unité exécute son ordre quelle que soit la vision révélée en cours de route (par elle ou par une autre unité). Chemin invalide = effacé ; blocage = gelé (repris plus tard) |
