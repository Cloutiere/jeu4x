...
| **École de Confucius** | **2 GP gratuits** (canon dataminé) — classes par **rotation R-127** (indices `greatPersonsObtained` + i), posés à la **capitale** (sinon première cité — case libre adjacente), **sans jalon** (miroir C2). **EXEMPTION D'ESCALADE (révision Calibrage canon — Erik 06/09)** : les GP de Confucius sont une **manne hors progression** — ils N'incrémentENT ni `greatPersonsObtained` (seuil culturel T-27) ni les compteurs T-30 ; le seuil culturel des GP suivants est **inchangé** (canon : « cet octroi n'augmente pas le coût culturel requis pour les GP suivants »). |
...
- **R-66 (rév. 06/09 — centre-ville, demande d'Erik) — socle garanti 1N / 1P / 1C** : la case de ville produit **au minimum 1 Nourriture, 1 Production et 1 Commerce** sur **N'IMPORTE QUEL terrain** (plancher par ressource 🔶, données `growth.json` `cityCenter.floor` — remplace le plancher « 1 P seul » de 7i). Le plancher s'applique **SUR les rendements calculés du centre** (terrain ville + bonus de bâtiments + traits de civilisation) : il ne plafonne rien — une ville Égypte sur désert peut dépasser 1 N via son trait. **Le commerce de tranche R-60bis s'ajoute AU-DESSUS du socle** (0 sous pop 7, jusqu'à +5 à pop 31) — **cette révision RÉSOUT le vote de calibrage 7i en suspens** (« commerce du centre-ville par tranche ») : le socle donne 1 C dès la fondation, la tranche s'empile ensuite. Le centre étant **toujours travaillé d'office** (non assignable, R-60), le plancher s'applique sans interaction avec l'auto-assignation — implémenté dans `tileYield` (source unique moteur/UI/3D) ;
...
**R-114 (rév. 7j, seuils rév. 7l · C5) · Personnages Illustres de culture.** Quand la culture accumulée d'une ville atteint le **seuil du canal culture** (T-27 **rév. C5 — table canon d'Erik**, voir ci-dessous) : un GP apparaît — unité **pacifique 0/0/2** (`greatPerson: true`), posé sur la **case de la ville** (sinon première case adjacente libre — perdu si aucune) ; événement `GreatPersonSpawned` ; la jauge est remise à zéro **par soustraction du seuil** (miroir R-63) ; au plus **un GP par ville et par tour** ; `greatPersonsObtained` est incrémenté. Les GP ne sont **jamais produites par les files** ; capturés en guerre ils sont détruits (R-43) **sans** jalon. **Révision 7j (D1 — le doc d'Erik tranche)** : l'**alternance Artiste/Penseur est OBSOLÈTE** — fusion en UNE classe **`artiste_penseur`** « Grand Artiste / Penseur » ; la classe engendrée suit le **ciblage technologique R-127**. Voir §8.8.
**7l · C5 — Table canon des seuils culturels (révision T-27, décision d'Erik du 05/09)** : le « 20 ×2 » de 7f est **remplacé** par la **table canon d'Erik** — 1er GP à **150** points de culture, l'écart entre paliers croissant d'environ +33,33 à chaque GP obtenu (ancres d'Erik : 150, 267, 417, 600, 817, 1067, 1350, 1667, 2017, 2400, …, 15e = 4817, 20e = 8067). Implémentée en **table data-driven** (`culture.json` `greatPersonCultureThresholds`, générée par la formule « écart initial 117, croissance +33,33 par pas, arrondi » — contrôlée en tests contre les 12 valeurs d'ancrage ; extrapolation de la formule au-delà de la table). Les seuils d'accumulateurs (T-30) et Leader (T-31) restent en l'état (aucune source contraire).
...
**Jalon culturel — RÉVISÉ 7k (C2 — veto d'Erik du 04/09, révision de R-126/D3/D4).** **Seuls les GP issus du canal CULTURE** (seuil T-27, ciblage R-127 — Humanitaire compris depuis C1) comptent **immédiatement** comme un **Jalon Culturel** (`reason: 'obtain'`). Un GP issu d'un **accumulateur** (Savant/Explorateur/Bâtisseur, T-30), du **canal combat** (Leader, T-31) ou du **Premier découvrir** (D5.1) **NE compte PAS**. **Les merveilles continuent de compter** (R-115/R-131 — le doc est explicite). **Écart volontaire avec la ligne générale du doc** (« chaque GP obtenu compte automatiquement comme un Jalon Culturel ») : **décision d'Erik datée du 04/09**, consignée ici comme révision de R-126. L'installation (Settle) n'accorde pas de jalon (inchangé). L'escalade `T-27`/`T-30` reste alimentée par **toute** obtention de GP (tous canaux — inchangé).

**GP installés (D4.2/D4.3).** Champ **`city.settledGreatPersons: string[]`** — liste des classes installées dans la cité (source unique des multiplicateurs Settle R-126, du détail UI et du vol). **Seuls les installés émettent leurs bonus** (multiplicateurs appliqués en Phase C aux rendements de la cité hôte) ; un GP en attente de choix n'émet rien et **ne peut pas être volé** 🔶. Le vol d'installé (R-119 révisée) retire le GP le plus récemment installé de la ville cible et le réinstalle d'office dans la **capitale du voleur** (sinon première ville — le bonus Settle change de camp ; −1/+1 jalon inchangé, escalade inchangée, décision d'Erik).
...
**R-127 · Ciblage technologique 🔶 (D5.2).** La classe du GP engendré par le **canal culture** est celle de la figure rattachée à la tech **en cours de recherche** du joueur (`player.researching` → `figures.json`) ; si la tech n'a aucune figure, **rotation déterministe** sur les 6 classes (index = `greatPersonsObtained`). Aucun RNG (R-80) — pondération déterministe par défaut du handoff (l'alternative tirage seedé reste possible, non retenue). Les autres canaux déterminent leur classe par nature (accumulateurs, combat).
