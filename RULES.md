...
- **dotation initiale `T-50` = 1 (POLISSAGE-1 C3, décision Erik 07/09)** : **1 barbare dans le camp au début de la partie**, posé sur une case adjacente libre (tri `(q, r)` — R-81), inscrit dans `spawnedUnits` (il compte pour le cap et la garde) ;
- **réengendrement** : +1 unité toutes les `T-18` 🔶 tours (compteur `spawnCountdown` initialisé à `T-18`, décrémenté à chaque résolution ; premier réengendrement au **tour 10**) tant que son **cap d'unités vivantes** `T-22` 🔶 (par village, suivi par `spawnedUnits`) n'est pas atteint — **le camp se régénère à 3 sans jamais le dépasser** ; l'unité apparaît sur une **case adjacente libre** du village (tri `(q, r)` — R-81 ; un défenseur ne peut donc pas camper sur le village et le rendre inexpugnable) — si aucune case adjacente n'est libre, l'engendrement est **reporté** au cycle suivant ;
...
**R-99 · Données de calibrage.** Toutes les constantes barbares/huttes vivent dans **`barbares.json`** (`spawnInterval`, `aggroRadius`, `gardeMinimale` (T-49), `initialUnits` (T-50), `villageDestructionGold`, `villageHP`, `capPerVillage`, `escalationTurn`, `units` d'escalade, `villageDefense`) et **`huttes.json`** (table de récompenses pondérée) — **zéro durcissement de règle dans le code** : calibrer = éditer le JSON + push (CI déploie), même philosophie que R-86/R-91. `constants.ts` ré-exporte les valeurs (source unique des T-18..T-26 et T-49/T-50 côté code). Tests d'intégrité : table de récompenses fermée (kinds connus), poids ≥ 0 et somme > 0, bornes or cohérentes (`min ≤ max`), unités d'escalade existantes.
