...
- **R-103 · Fertilité & normalisation** : score = Σ anneaux 1/2/3 pondérés 🔶 [1.0, 0.6, 0.3] × (nourriture ×2 + production ×1.5 + commerce ×1), pénalité 🔶 par montagne (la montagne reste travaillable R-60) ; ressources incluses. Site de capitale = meilleur candidat praticable (≥ 6 des bords, ≥ T-09 de l'axe, distance au site image ≥ 12, ≥ 1 voisin libre pour le Guerrier). **Normalisation** (PDF §NormalizeStartLocation) : si le site est sous le seuil 🔶 (moyenne des N=5 meilleurs sites × 0.8), injecter bétail/blé (données R-91) dans les anneaux 1-2 jusqu'au seuil — échec explicite sinon. Checksum d'équité (delta P1−P2 = 0 par miroir, score ≥ seuil) consigné dans `meta.progen` → dump admin.
