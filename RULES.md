...
**R-114 (rév. 7j) · Personnages Illustres de culture.** Quand la culture accumulée d'une ville atteint le **seuil `T-27`** (base 🔶 20, **×2** à chaque GP obtenu **par l'empire** — `player.greatPersonsObtained`) : un GP apparaît — unité **pacifique 0/0/2** (`greatPerson: true`), posé sur la **case de la ville** (sinon première case adjacente libre — perdu si aucune) ; événement `GreatPersonSpawned` ; la jauge est remise à zéro **par soustraction du seuil** (miroir R-63) ; au plus **un GP par ville et par tour** ; `greatPersonsObtained` est incrémenté — le seuil monte pour TOUTES les villes de l'empire. Les GP ne sont **jamais produites par les files** ; capturés en guerre ils sont détruits (R-43) **sans** jalon. **Révision 7j (D1 — le doc d'Erik tranche)** : l'**alternance Artiste/Penseur est OBSOLÈTE** — fusion en UNE classe **`artiste_penseur`** « Grand Artiste / Penseur » ; la classe engendrée suit le **ciblage technologique R-127**. Voir §8.8.
...
**R-129 · Exclusivité mondiale (M2 — doc : « une Merveille ne peut être construite qu'une seule fois par partie, toutes civilisations confondues ; seule la première à cumuler le nombre de marteaux requis valide l'édifice »).** La validation `wonderProductionIssue` refuse une merveille **déjà bâtie n'importe où** (toute ville, tout propriétaire — révision de l'unicité d'empire 7f, qui reste vraie par inclusion) ; le chantier concurrent chez le rival reste autorisé (la course est légitime). **Complétion simultanée au même tour** : les villes sont traitées en `(cityId` croissant — R-81 🔶 `)`, la **première** dans cet ordre valide, la seconde est un no-op documenté (et bascule en récupération R-130 si son propriétaire est un rival du compléteur). La merveille **survit à la capture** (R-115 — inchangé, ne pas toucher sauf bug).

**R-130 · Récupération des marteaux (M3 — doc : « les marteaux déjà investis ne sont pas perdus à condition d'être réaffectés à un autre projet durant le même tour ; sinon dissipés »).** Adaptation aux **tours simultanés** 🔶 (défaut retenu, décision d'agent à calibrer) : à la résolution où un rival complète la merveille que je construisais, **toute ville** (de tout joueur — moi y compris si double complétion refusée) qui avait cette merveille en chantier voit sa production basculer en **récupération en attente** : nouveau champ de ville **`pendingSalvage`** (marteaux conservés, production vidée), événement **`HammerSalvage { cityId, owner, amount, outcome: 'available' | 'dissipated' }`** 🔶 (libellé calibrable). Le joueur **réaffecte pendant son tour suivant** via un `SetProduction` ordinaire (aucun nouvel ordre — la forme `orderShapeError` est inchangée) : la progression du nouveau projet **démarre à `pendingSalvage`** (conservation intégrale, miroir de « progression conservée » R-62 — 🔶 sans plafond au coût du nouvel item). **À la résolution suivante** (fenêtre = **1 tour** 🔶, `culture.json` `hammerSalvageWindow` — calibrable), les marteaux non réaffectés sont **dissipés** (`outcome: 'dissipated'`). Interprétations 🔶 documentées : la continuation forcée de combat (R-55/R-56-3) et les attaques barbares ne sont pas des « attaques initiées » — voir R-133 ; la capture de la ville dissipe la récupération (les marteaux n'appartiennent pas au captreur).
...
| **Cie des Indes Orientales** | `oceanCommerceBonus: 1` | **+1 Commerce sur chaque case océanique (`ocean`) exploitée** par la ville hôte (bonus par terrain travaillé, modèle R-66 — intégré à `tileYield`, auto-assignation comprise) | 🔶 libellé : le doc dit « maritime », le handoff « océanique » — `ocean` seul retenu (côte exclue) |
...
| **Foire de Troyes** | `cityGoldMult: 2` | **×2 la production d'Or de la cité** — interprétation 🔶 : multiplie la part **or** de la conversion R-90 de la cité (la trésorerie existe, le rush-buy non — 7l) | s'applique avant le multiplicateur Settle Explorateur (round half up final) |
| **Complexe Militaro-Industriel** | `militaryCostMult: 0.8` | **−20 % le coût de production des unités militaires** de l'empire (production seule pour l'instant) | le « coût d'achat » du libellé data concernera le **rush-buy (7l)** |
| **Internet** | `empireGoldMult: 2` | **×2 la production d'Or dans tout l'empire** — même interprétation 🔶 que Foire de Troyes (part or de la conversion R-90, toutes villes) | cumul avec Troyes : **MAX** (convention des multiplicateurs, R-88) 🔶 |
| **Programme Apollo** | `allTechsOnComplete` | Accorde **instantanément l'ensemble des technologies** de l'arbre (octroi direct — ni `firstBy` ni récompenses ; conséquence canonique : obsolescence globale R-128 de toutes les merveilles concernées) | effet massif endgame, implémenté tel quel 🔶 |
...
Hors périmètre 7k : **Projet Manhattan/ICBM** (7m), **Banque mondiale** (7l), **SETI et merveilles DLC** (exclus v1).
...
- **Nations Unies** (victoire à 20 jalons) : conforme mécaniquement (verrou < 20 jalons, complétion = victoire) ; **écart de coût 🔶 signalé** : catalogue du doc 500 marteaux vs notre `T-28` 300 🔶 (calibrage 7f — non modifié sans validation d'Erik).
...
| T-32 | `hammerSalvageWindow` | 1 🔶 (R-130, Phase 7k — `culture.json` : tours de réaffectation avant dissipation des marteaux récupérés) |
