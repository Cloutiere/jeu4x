...
| Grand Humanitaire (`humanitaire`) | accumulateur CROISSANCE T-30 (`city.gpAccumFood` — excédent alimentaire cumulé 🔶) | **+1 pop à TOUTES les cités** de l'empire | **+50 % croissance** (surplus alimentaire ×1,5) de la cité hôte |
...
**R-126 · Consume / Settle.** Chaque GP obtenu reste sur la carte **« en attente de choix »** ; le joueur le résout via le nouvel ordre **`GreatPersonAction { unitId, action: 'consume' | 'settle', cityId }`** (Phase C — `cityId` = ville amie sur sa case ou adjacente, comme R-115). Les effets sont EXACTEMENT ceux du tableau ci-dessus (le doc fait foi). Un ordre invalide (ville ennemie, trop loin, effet inactif/impossible) est **ignoré** — le GP reste en attente. `InstallPerson` (R-115) reste accepté comme **alias de `Settle`** (compat clients 7f/7h). Interprétations 🔶 documentées : multiplicateurs Settle **additifs** (+50 % par GP installé de la classe ; remise Bâtisseur ×0,5 par GP, multiplicative) ; le jalon est compté **À L'OBTENTION** (voir ci-dessous) — aucun des deux choix n'en accorde. Le bot applique un **choix déterministe simple 🔶 : toujours Settle**.

**Jalon À L'OBTENTION (D3/D4 — révise R-115).** Chaque GP obtenu (toutes classes, tous canaux — Premier découvrir inclus) compte **immédiatement** comme un **Jalon Culturel** (`reason: 'obtain'`) : le doc — « chaque Grand Personnage obtenu compte automatiquement comme un Jalon Culturel ». L'installation (Settle) n'accorde plus de jalon. L'escalade `T-27` est désormais alimentée par **toute** obtention de GP (« le seuil augmente à chaque nouveau personnage » 🔶 — les GP d'accumulateurs font aussi monter le seuil culturel).
...
**D5.1 · Premier découvrir accorde un GP (R-109 étendu).** La récompense `firstToDiscover.greatPerson` (Invention → Grand Bâtisseur, Monarchie → Grand Leader) est **ACTIVE** : le GP de la classe de la figure de la tech est posé sur la première ville (sinon adjacente libre), jalon d'obtention compté, événement `FirstDiscovered` (champ `greatPerson`). Les canaux or (500 / 10 000) restent **reportés 7l**.
