...
describe('L2 · GameState versionné (DESIGN.md §3.8)', () => {
  it('la version courante est exportée avec la chaîne de migrations (v12 depuis les gouvernements, Phase 7h)', () => {
    ...
    expect(CURRENT_SCHEMA_VERSION).toBe(14); // 7k : merveilles (R-128..R-132, salvage)
  ...
  it('v8 → v9 (Phase 7e) : firstBy vide + Palais posé dans les capitales, idempotent', () => {
    ...
    expect(out.schemaVersion).toBe(14); // la chaîne se poursuit jusqu'à la version courante (7h)
  ...
  it('migrateState rejette une version inconnue ou futuriste', () => {
    ...
    expect(() => migrateState({ schemaVersion: 15 })).toThrow(); // 14 = version courante (7k)
  ...
  it('v7 → v9 : villages/huttes/mapId additifs puis firstBy/Palais (chaîne 7d + 7e)', () => {
    ...
    expect(out.schemaVersion).toBe(14); // chaîne complète 7d → 7e → 7f
