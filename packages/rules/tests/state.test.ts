...
describe('L2 · GameState versionné (DESIGN.md §3.8)', () => {
  it('la version courante est exportée avec la chaîne de migrations (v12 depuis les gouvernements, Phase 7h)', () => {
    ...
    expect(CURRENT_SCHEMA_VERSION).toBe(26); // ENGAGEMENT (R-173 : champ stabilized, migration 24→25) : cultureStored supprimé + culturePaliers (D1/D5)
  ...
  it('v8 → v9 (Phase 7e) : firstBy vide + Palais posé dans les capitales, idempotent', () => {
    ...
    expect(out.schemaVersion).toBe(26); // la chaîne se poursuit jusqu'à la version courante (7m)
  ...
  it('migrateState rejette une version inconnue ou futuriste', () => {
    ...
    expect(() => migrateState({ schemaVersion: 27 })).toThrow(); // 26 est la version courante (CARTE-MULTI — defeated)
  ...
  it('v7 → v9 : villages/huttes/mapId additifs puis firstBy/Palais (chaîne 7d + 7e)', () => {
    ...
    expect(out.schemaVersion).toBe(26); // chaîne complète 7d → … → 7o → GP-CULTURE-EVENEMENTS
