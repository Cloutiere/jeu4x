// HABILLER-GALLION — adaptation du gallion Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-gallion.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-gallion.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-gallion.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-gallion.mjs              peint + exporte modeles/gallion_v3.glb (+ cartes à plat)
//
// Source : image_ref/gallion_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/gallion_tripo.glb';
const SORTIE = '../modeles/gallion_v3.glb';
...
const S = {
  pivot180: false,         // l'alignement se fait via axeFuselage (coque diagonale ~53° dans le source)
  axeFuselage: [0.802, 0, -0.597], // direction proue (PCA horizontale) => alignée sur -Z
  nivelerRoulis: false,
  poserAuSol: true,
  ...
  teteY: [0.0, 0.53],
  teteZmin: 0.20,          // château/poupe (z > 0.20)
  lame: { zMax: -0.22 },   // éperon de proue (z < -0.22)
...
// ---------- alignement 3D de l'axe de la coque sur -Z (proue, demande du lot) ----------
if (S.axeFuselage) {
  const norm = (v) => { const l = Math.hypot(...v); return [v[0]/l, v[1]/l, v[2]/l]; };
  const cross = (a, b) => [a[1]*b[2]-a[2]*b[1], a[2]*b[0]-a[0]*b[2], a[0]*b[1]-a[1]*b[0]];
  const f = norm(S.axeFuselage);
  ...
  if (S.poserAuSol) {
    ...
    console.log('coque alignée sur -Z, posée au sol');
  }
}
if (false) {
  const norm = (v) => { const l = Math.hypot(...v); return [v[0]/l, v[1]/l, v[2]/l]; };
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  if (cy >= S.teteY[0] && cy <= S.teteY[1] && cz > S.teteZmin) zone[t] = Z.VISIERE;
  else if (cz < S.lame.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/gallion-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/gallion-*.png');
  process.exit(0);
}
...
writeFileSync(new URL('../captures/gallion-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/gallion-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/gallion-carte-base.png + gallion-carte-emissive.png');
