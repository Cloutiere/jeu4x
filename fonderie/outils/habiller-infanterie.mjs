// HABILLER-INFANTERIE — adaptation de l'infanterie Tripo (trooper au bouclier) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-infanterie.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-infanterie.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-infanterie.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-infanterie.mjs              peint + exporte modeles/infanterie_v3.glb (+ cartes à plat)
//
// Source : image_ref/infanterie_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/infanterie_tripo.glb';
const SORTIE = '../modeles/infanterie_v3.glb';
...
const S = {
  ...
  teteY: [0.85, 1.01],
  teteXmax: 0.25,          // visière du casque (face avant z<0)
  teteZmin: -0.05,
  bouclier: { xMax: -0.28 }, // grand bouclier (toute hauteur)
  smg: { x0: 0.20, y0: 0.25, y1: 0.60, zMax: -0.12 }, // SMG (avant-droit)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=visière, LAME=bouclier+SMG
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cx < S.bouclier.xMax) zone[t] = Z.LAME;
  else if (cx > S.smg.x0 && cy > S.smg.y0 && cy < S.smg.y1 && cz < S.smg.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/infanterie-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/infanterie-*.png');
  process.exit(0);
}

// lignes néon (visière + bouclier + SMG) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : visière + bouclier + SMG (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/infanterie-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/infanterie-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/infanterie-carte-base.png + infanterie-carte-emissive.png');
