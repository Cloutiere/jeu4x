// HABILLER-KNIGHT — adaptation du knight Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-HABILLAGE-TRIPO.
// Usage :
//   node outils/habiller-knight.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-knight.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-knight.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-knight.mjs              peint + exporte modeles/knight_v3.glb (+ cartes à plat)
//
// Source : image_ref/glowing+knight+3d+model.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/glowing+knight+3d+model.glb';
const SORTIE = '../modeles/knight_v3.glb';
...
const S = {
  pivot180: true,          // confirmé --preview : le knight regarde +Z => pivot pour face -Z
  ...
  // zones (espace modèle APRÈS pivot + recentrage, AVANT échelle) :
  teteY: [0.78, 1.01],
  visiereZmax: -0.06,      // visière = face avant de la tête (z < seuil)
  lame: { x0: 0.16, y0: 0.42, z0: -0.15 }, // lame = x>x0, y>y0, z<z0 (bras/gant exclus par z)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 };
const zone = new Uint8Array(NB_TRIS);
for (let t = 0; t < NB_TRIS; t++) {
  const cx = centroid[t * 3], cy = centroid[t * 3 + 1], cz = centroid[t * 3 + 2];
  if (cy >= S.teteY[0] && cy <= S.teteY[1] && cz < S.visiereZmax) zone[t] = Z.VISIERE;
  else if (cx > S.lame.x0 && cy > S.lame.y0 && cz < S.lame.z0) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    lignes,
    vues: VUES,
    destBase: 'captures/knight',
    palette: [[0.62, 0.78, 0.72], [1.0, 0.35, 0.35], [0.3, 0.45, 0.95]],
    nomFichier: (v) => `../captures/knight-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/knight-*.png');
  process.exit(0);
}

// lignes néon (visière + tranchant) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : visière + tranchant (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/knight-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/knight-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/knight-carte-base.png + knight-carte-emissive.png');
