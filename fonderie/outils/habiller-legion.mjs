// HABILLER-LEGION — adaptation de la legion Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-legion.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-legion.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-legion.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-legion.mjs              peint + exporte modeles/legion_v3.glb (+ cartes à plat)
//
// Source : image_ref/legion_tripod_2.0.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/legion_tripod_2.0.glb';
const SORTIE = '../modeles/legion_v3.glb';
...
const S = {
  ...
  teteY: [0.75, 0.93],
  teteXmax: 0.15,          // visage de la tête massive voûtée
  teteZmin: -0.15,
  lame: { x0: 0.32, y1: 0.60, zMax: -0.05 }, // gladius vertical (colonne avant-droite)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=visage, LAME=gladius
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cx > S.lame.x0 && cy < S.lame.y1 && cz < S.lame.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/legion-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/legion-*.png');
  process.exit(0);
}

// lignes néon (visage + gladius) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : visage + gladius (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/legion-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/legion-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/legion-carte-base.png + legion-carte-emissive.png');
