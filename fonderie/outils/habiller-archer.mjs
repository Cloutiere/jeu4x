// HABILLER-ARCHER — adaptation de l'archer Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-archer.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-archer.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-archer.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-archer.mjs              peint + exporte modeles/archer_v3.glb (+ cartes à plat)
//
// Source : image_ref/archer_tripo_2.0.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/archer_tripo_2.0.glb';
const SORTIE = '../modeles/archer_v3.glb';
...
const S = {
  ...
  teteY: [0.75, 1.01],
  teteXmax: 0.05,          // visage (exclut l'arrière de la tête et le bras d'arc)
  teteZmin: -0.02,
  arc: { xMax: -0.25, y0: 0.35, zMax: 0.10 }, // arc vertical + flèche (colonne avant-gauche)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=visage, LAME=arc/flèche
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cx < S.arc.xMax && cy > S.arc.y0 && cz < S.arc.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/archer-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/archer-*.png');
  process.exit(0);
}

// lignes néon (visage + arc) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : visage + arc (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/archer-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/archer-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/archer-carte-base.png + archer-carte-emissive.png');
