// HABILLER-ESPION — adaptation de l'espion Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-espion.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-espion.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-espion.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-espion.mjs              peint + exporte modeles/espion_v3.glb (+ cartes à plat)
//
// Source : image_ref/espion_tripod.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/espion_tripod.glb';
const SORTIE = '../modeles/espion_v3.glb';
...
const S = {
  ...
  teteY: [0.55, 0.76],
  teteXmax: 0.15,          // visage bas sous la capuche
  teteZmin: -0.32,
  lame: { x0: 0.10, y0: 0.28, y1: 0.50, zMax: -0.18 }, // dague (avant-droite)
  outil: { xMax: -0.40, y0: 0.28, y1: 0.50, zMax: 0.0 }, // outil d'injection (avant-gauche)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=visage, LAME=dague+outil
const zone = new Uint8Array(NB_TRIS);
for (let t = 0; t < NB_TRIS; t++) {
  const cx = centroid[t * 3], cy = centroid[t * 3 + 1], cz = centroid[t * 3 + 2];
  if (cy >= S.teteY[0] && cy <= S.teteY[1] && cz < S.teteZmin && cx < S.teteXmax) zone[t] = Z.VISIERE;
  else if (cx > S.lame.x0 && cx < 0.45 && cy > S.lame.y0 && cy < S.lame.y1 && cz < S.lame.zMax) zone[t] = Z.LAME;
  else if (cx < S.outil.xMax && cy > S.outil.y0 && cy < S.outil.y1 && cz < S.outil.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/espion-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/espion-*.png');
  process.exit(0);
}

// lignes néon (visage + dague + outil) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : visage + dague + outil (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/espion-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/espion-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/espion-carte-base.png + espion-carte-emissive.png');
