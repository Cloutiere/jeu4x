// HABILLER-CATAPULTE — adaptation de la catapulte Tripo (lance-missiles lourd) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-catapulte.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-catapulte.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-catapulte.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-catapulte.mjs              peint + exporte modeles/catapulte_v3.glb (+ cartes à plat)
//
// Source : image_ref/catapulte_tripod.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/catapulte_tripod.glb';
const SORTIE = '../modeles/catapulte_v3.glb';
...
const S = {
  ...
  teteY: [0.15, 0.35],
  teteXmax: 0.20,          // « visage » = face avant de la caisse (cabine/vents)
  teteZmin: -0.28,
  tube: { x0: -0.12, x1: 0.12, y0: 0.35 }, // tube lanceur central + bouche
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=cabine, LAME=tube lanceur
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cx > S.tube.x0 && cx < S.tube.x1 && cy > S.tube.y0) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/catapulte-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/catapulte-*.png');
  process.exit(0);
}

// lignes néon (cabine + tube) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : cabine + tube (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/catapulte-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/catapulte-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/catapulte-carte-base.png + catapulte-carte-emissive.png');
