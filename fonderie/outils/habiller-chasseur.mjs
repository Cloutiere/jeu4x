// HABILLER-CHASSEUR — adaptation du chasseur Tripo (redressement du nez) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-chasseur.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-chasseur.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-chasseur.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-chasseur.mjs              peint + exporte modeles/chasseur_v3.glb (+ cartes à plat)
//
// Source : image_ref/chasseur_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/chasseur_tripo.glb';
const SORTIE = '../modeles/chasseur_v3.glb';
...
const S = {
  pivot180: false,         // inutilisé pour le chasseur (l'alignement se fait via alignerAxe)
  axeFuselage: [0.573, 0.451, -0.684], // axe PCA du fuselage (vers le nez) : lacet ~40°, cabré ~27°
  nivelerRoulis: true,     // aile droite 11% plus haute dans le source
  poserAuSol: true,        // après alignement, min y = 0           // confirmé --preview : visée ~-Z après rotation +90° Y (pose Tripo biaisée ~25° vers +X)
  ...
  teteY: [0.15, 0.29],
  teteXmax: 0.12,          // nez/canopée (pointe avant)
  teteZmin: -0.25,
  lame: { y1: 0.12, x0: 0.18, zMin: -0.20 }, // missiles sous les ailes (|x|>x0, y bas, z arrière)
...
// ---------- alignement 3D de l'axe fuselage sur -Z (redressement du nez, demande d'Erik) ----------
if (S.axeFuselage) {
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=nez/canopée, LAME=missiles
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cy < S.lame.y1 && Math.abs(cx) > S.lame.x0 && cz > S.lame.zMin) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/chasseur-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/chasseur-*.png');
  process.exit(0);
}

// lignes néon (nez + missiles) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : nez + missiles (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/chasseur-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/chasseur-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/chasseur-carte-base.png + chasseur-carte-emissive.png');
