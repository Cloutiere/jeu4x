// HABILLER-COLON — adaptation du colon Tripo (véhicule) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-colon.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-colon.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-colon.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-colon.mjs              peint + exporte modeles/colon_v3.glb (+ cartes à plat)
//
// Source : image_ref/colon_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/colon_tripo.glb';
const SORTIE = '../modeles/colon_v3.glb';
...
const S = {
  pivot90: true,           // le colon regarde -X (coque effilée à -X, confirmé par profil de largeur) => rotation +90° Y : -X => -Z
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=nez/cabine, LAME=turbines avant
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cz > S.griffeG.zMin && cy < S.griffeG.y1) zone[t] = Z.LAME;
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    lignes,
    vues: VUES,
    destBase: 'captures/colon',
    palette: [[0.62, 0.78, 0.72], [1.0, 0.35, 0.35], [0.3, 0.45, 0.95]],
    nomFichier: (v) => `../captures/colon-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/colon-*.png');
  process.exit(0);
}

// lignes néon (cabine + moteurs arrière) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : cabine + moteurs arrière (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/colon-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/colon-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/colon-carte-base.png + barbare-carte-emissive.png');
