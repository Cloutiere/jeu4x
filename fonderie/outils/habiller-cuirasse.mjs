// HABILLER-CUIRASSE — adaptation du cuirasse Tripo (navire de guerre) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-cuirasse.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-cuirasse.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-cuirasse.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-cuirasse.mjs              peint + exporte modeles/cuirasse_v3.glb (+ cartes à plat)
//
// Source : image_ref/cuirasse_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/cuirasse_tripo.glb';
const SORTIE = '../modeles/cuirasse_v3.glb';
...
const S = {
  pivot180: false,         // le lacet -90° ci-dessous suffit (proue à -X dans le source)
  lacetNeg90: true,        // proue -X => -Z (confirmé --preview : la vue « gauche » montre la proue)
  axeFuselage: null,
  nivelerRoulis: false,
  poserAuSol: true,           // confirmé --preview : visée ~-Z après rotation +90° Y (pose Tripo biaisée ~25° vers +X)
  ...
  teteY: [0.18, 0.28],
  teteXmax: 0.15,          // dôme passerelle central
  teteZmax: 0.15,
  lame: { y0: 0.10, zBande: 0.30 }, // canons des tourelles avant/arrière (|z| > 0.30)
...
// ---------- lacet -90° (proue -X => -Z) : x' = -z, z' = x ----------
if (S.lacetNeg90) {
  const tourne = (tab) => {
    for (let i = 0; i < tab.length; i += 3) {
      const x = tab[i], z = tab[i + 2];
      tab[i] = -z; tab[i + 2] = x;
    }
  };
  tourne(pos); tourne(nor);
  if (S.poserAuSol) {
    let minY = Infinity;
    for (let i = 1; i < pos.length; i += 3) if (pos[i] < minY) minY = pos[i];
    for (let i = 1; i < pos.length; i += 3) pos[i] -= minY;
    console.log(`lacet -90° posé au sol (min y ${minY.toFixed(3)})`);
  }
}

// ---------- alignement 3D de l'axe fuselage sur -Z (inutilisé ici) ----------
if (S.axeFuselage) {
  const norm = (v) => { const l = Math.hypot(...v); return [v[0]/l, v[1]/l, v[2]/l]; };
  const cross = (a, b) => [a[1]*b[2]-a[2]*b[1], a[2]*b[0]-a[0]*b[2], a[0]*b[1]-a[1]*b[0]];
  const f = norm(S.axeFuselage);              // vers le nez
  let r = norm(cross([0, 1, 0], f));          // droite horizontale
  let u = norm(cross(r, f));                  // « haut » du fuselage
  if (u[1] < 0) { r = [-r[0], -r[1], -r[2]]; u = [-u[0], -u[1], -u[2]]; } // u vers le haut, toujours
  const appliquer = (tab) => {
    for (let i = 0; i < tab.length; i += 3) {
      const x = tab[i], y = tab[i + 1], z = tab[i + 2];
      tab[i]     = x*r[0] + y*r[1] + z*r[2];
      tab[i + 1] = x*u[0] + y*u[1] + z*u[2];
      tab[i + 2] = -(x*f[0] + y*f[1] + z*f[2]); // le nez (le long de f) part en -Z
    }
  };
  appliquer(pos); appliquer(nor);
  // nivellement du roulis : les extrêmes x (saucissons d'aile) doivent être à la même hauteur
  if (S.nivelerRoulis) {
    let gx = Infinity, gX = -Infinity;
    for (let i = 0; i < pos.length; i += 3) { if (pos[i] < gx) gx = pos[i]; if (pos[i] > gX) gX = pos[i]; }
    let yg = 0, ng = 0, yd = 0, nd = 0;
    for (let i = 0; i < pos.length; i += 3) {
      if (pos[i] < gx + 0.08) { yg += pos[i + 1]; ng++; }
      if (pos[i] > gX - 0.08) { yd += pos[i + 1]; nd++; }
    }
    const phi = Math.atan2((yd/nd) - (yg/ng), gX - gx);
    const c = Math.cos(phi), s2 = Math.sin(phi);
    for (let i = 0; i < pos.length; i += 3) {
      const y = pos[i + 1], z = pos[i + 2];
      pos[i + 1] = y * c - z * s2; pos[i + 2] = y * s2 + z * c;
    }
    for (let i = 0; i < nor.length; i += 3) {
      const y = nor[i + 1], z = nor[i + 2];
      nor[i + 1] = y * c - z * s2; nor[i + 2] = y * s2 + z * c;
    }
    console.log(`roulis nivelé : ${(phi * 180 / Math.PI).toFixed(1)}°`);
  }
  if (S.poserAuSol) {
    let minY = Infinity;
    for (let i = 1; i < pos.length; i += 3) if (pos[i] < minY) minY = pos[i];
    for (let i = 1; i < pos.length; i += 3) pos[i] -= minY;
    console.log(`axe aligné sur -Z, posé au sol (min y ${minY.toFixed(3)})`);
  }
}
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=dôme passerelle, LAME=canons
const zone = new Uint8Array(NB_TRIS);
for (let t = 0; t < NB_TRIS; t++) {
  const cx = centroid[t * 3], cy = centroid[t * 3 + 1], cz = centroid[t * 3 + 2];
  if (cy >= S.teteY[0] && cy <= S.teteY[1] && Math.abs(cx) < S.teteXmax && Math.abs(cz) < S.teteZmax) zone[t] = Z.VISIERE;
  else if (cy > S.lame.y0 && Math.abs(cz) > S.lame.zBande) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/cuirasse-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/cuirasse-*.png');
  process.exit(0);
}

// lignes néon (dôme + canons) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : dôme + canons (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/cuirasse-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/cuirasse-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/cuirasse-carte-base.png + cuirasse-carte-emissive.png');
