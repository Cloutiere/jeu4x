// HABILLER-ARTILLERIE — adaptation de l'artillerie Tripo (rail-cannon) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-artillerie.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-artillerie.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-artillerie.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-artillerie.mjs              peint + exporte modeles/artillerie_v3.glb (+ cartes à plat)
//
// Source : image_ref/artillerie_tripod.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/artillerie_tripod.glb';
const SORTIE = '../modeles/artillerie_v3.glb';
...
const S = {
  ...
  teteY: [0.05, 0.20],
  teteXmax: 0.18,          // face avant de la caisse
  teteZmin: -0.12,
  tube: { x0: -0.09, x1: 0.09, y0: 0.20 }, // canon central + frein de bouche
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/artillerie-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/artillerie-*.png');
  process.exit(0);
}
...
writeFileSync(new URL('../captures/artillerie-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/artillerie-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/artillerie-carte-base.png + artillerie-carte-emissive.png');
