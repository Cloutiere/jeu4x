// HABILLER-GALERE — adaptation de la galere Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-galere.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-galere.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-galere.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-galere.mjs              peint + exporte modeles/galere_v3.glb (+ cartes à plat)
//
// Source : image_ref/galere_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/galere_tripo.glb';
const SORTIE = '../modeles/galere_v3.glb';
...
const S = {
  ...
  lacetPos90: false,
  lacetNeg90: true,        // proue -X => -Z (comme le cuirasse) ; vaisseau quasi symétrique
  axeFuselage: null,
  nivelerRoulis: false,
  ...
  teteY: [0.06, 0.17],
  teteXmax: 0.14,          // tambour central (roue de propulsion)
  teteZmax: 0.14,
  lame: { y0: 0.03, zBande: 0.32 }, // extrémités proue/poupe
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/galere-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/galere-*.png');
  process.exit(0);
}
...
writeFileSync(new URL('../captures/galere-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/galere-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/galere-carte-base.png + galere-carte-emissive.png');
