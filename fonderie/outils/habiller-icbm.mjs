// HABILLER-ICBM — adaptation du missile ICBM Tripo (couché, nez -Z) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-icbm.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-icbm.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-icbm.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-icbm.mjs              peint + exporte modeles/icbm_v3.glb (+ cartes à plat)
//
// Source : image_ref/ICBM_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/ICBM_tripo.glb';
const SORTIE = '../modeles/icbm_v3.glb';
...
const S = {
  axeFuselage: [0.804, 0, 0.595], // direction nez (PCA horizontale, missile diagonal ~36°) => alignée sur -Z
  ...
  dihedreNeon: 25,         // degré => arête néon LINES (zones pleines)
  ...
  teteY: [0.0, 0.55],
  teteZmin: -0.45,         // cône de nez (ogive)
  tuyeres: { zMin: 0.42 }, // tuyères + empennage
...
// ---------- alignement 3D de l'axe du missile sur -Z (nez) ----------
if (S.axeFuselage) {
  ...
  if (u[1] < 0) { r = [-r[0],-r[1],-r[2]]; u = [-u[0],-u[1],-u[2]]; }
  const appliquer = (tab) => {
  ...
  // redressement du tangage résiduel (missile cabré dans le source) : nez et queue à même hauteur
  {
    let ny = 0, nn = 0, ty = 0, tn = 0, zN = 0, zT = 0;
    for (let i = 0; i < pos.length; i += 3) {
      if (pos[i + 2] < -0.40) { ny += pos[i + 1]; zN += pos[i + 2]; nn++; }
      if (pos[i + 2] > 0.40) { ty += pos[i + 1]; zT += pos[i + 2]; tn++; }
    }
    ny /= nn; ty /= tn; zN /= nn; zT /= tn;
    const pitch = Math.atan2(ny - ty, zT - zN); // > 0 si le nez est haut
    const c = Math.cos(-pitch), s2 = Math.sin(-pitch);
    for (let i = 0; i < pos.length; i += 3) {
    ...
    console.log(`tangage résiduel corrigé : ${(pitch * 180 / Math.PI).toFixed(1)}°`);
  }
  if (S.poserAuSol === undefined || S.poserAuSol) {
    let minY = Infinity;
    for (let i = 1; i < pos.length; i += 3) if (pos[i] < minY) minY = pos[i];
    for (let i = 1; i < pos.length; i += 3) pos[i] -= minY;
    console.log('missile aligné (nez -Z), posé au sol');
  }
}
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=nez/ogive, LAME=tuyères
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cz > S.tuyeres.zMin) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    lignes,
    vues: VUES,
    destBase: 'captures/icbm',
    palette: [[0.62, 0.78, 0.72], [1.0, 0.35, 0.35], [0.3, 0.45, 0.95]],
    nomFichier: (v) => `../captures/icbm-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/icbm-*.png');
  process.exit(0);
}

// lignes néon (nez + tuyères) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : nez + tuyères (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/icbm-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/icbm-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/icbm-carte-base.png + icbm-carte-emissive.png');
