// HABILLER-CHEVALIER — adaptation du chevalier Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-chevalier.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-chevalier.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-chevalier.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-chevalier.mjs              peint + exporte modeles/chevalier_v3.glb (+ cartes à plat)
//
// Source : image_ref/chevalier_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/chevalier_tripo.glb';
const SORTIE = '../modeles/chevalier_v3.glb';
...
const S = {
  ...
  teteY: [0.66, 0.82],
  teteXmax: 0.05,          // visière du casque
  teteZmin: -0.02,
  lame: { x0: 0.15, y0: 0.32, y1: 0.58, zMax: 0.05 }, // grande lance (avant-droit)
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cx > S.lame.x0 && cy > S.lame.y0 && cy < S.lame.y1 && cz < S.lame.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/chevalier-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/chevalier-*.png');
  process.exit(0);
}
...
writeFileSync(new URL('../captures/chevalier-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/chevalier-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/chevalier-carte-base.png + chevalier-carte-emissive.png');
