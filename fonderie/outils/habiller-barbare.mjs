// HABILLER-BARBARE — adaptation du barbare Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-barbare.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-barbare.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-barbare.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-barbare.mjs              peint + exporte modeles/barbare_v3.glb (+ cartes à plat)
//
// Source : image_ref/barbare_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/barbare_tripo.glb';
const SORTIE = '../modeles/barbare_v3.glb';
...
const S = {
  pivot90: true,           // confirmé --preview : le barbare regarde +X => rotation +90° Y pour face -Z : le knight regarde +Z => pivot pour face -Z
  ...
  teteY: [0.78, 1.01],
  teteZmin: -0.12,         // face avant de la tête (tête basse, voûtée vers -Z)
  griffeG: { xMax: -0.28, y0: 0.45, zMax: -0.30 }, // bras-griffe gauche tendi vers l'avant
  griffeD: { x0: 0.15, y0: 0.35, y1: 0.65, z0: 0.0 }, // bras droit ramené en arrière
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=tête, LAME=griffes
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cx < S.griffeG.xMax && cy > S.griffeG.y0 && cz < S.griffeG.zMax) zone[t] = Z.LAME;
  else if (cx > S.griffeD.x0 && cy > S.griffeD.y0 && cy < S.griffeD.y1 && cz > S.griffeD.z0) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    lignes,
    vues: VUES,
    destBase: 'captures/barbare',
    palette: [[0.62, 0.78, 0.72], [1.0, 0.35, 0.35], [0.3, 0.45, 0.95]],
    nomFichier: (v) => `../captures/barbare-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/barbare-*.png');
  process.exit(0);
}

// lignes néon (tête + griffes) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : tête + griffes (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/barbare-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/barbare-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/barbare-carte-base.png + barbare-carte-emissive.png');
