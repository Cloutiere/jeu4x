// HABILLER-CHAR — adaptation du char Tripo (double canon) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-char.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-char.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-char.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-char.mjs              peint + exporte modeles/char_v3.glb (+ cartes à plat)
//
// Source : image_ref/char_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/char_tripo.glb';
const SORTIE = '../modeles/char_v3.glb';
...
const S = {
  ...
  teteY: [0.10, 0.30],
  teteXmax: 0.25,          // glacis avant de la caisse
  teteZmin: -0.28,
  tube: { y0: 0.30, x0: -0.16, x1: 0.16, zMax: -0.05 }, // double canon (x ≈ ±0.10)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=glacis, LAME=canons
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cy > S.tube.y0 && cx > S.tube.x0 && cx < S.tube.x1 && cz < S.tube.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/char-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/char-*.png');
  process.exit(0);
}

// lignes néon (glacis + canons) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : glacis + canons (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/char-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/char-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/char-carte-base.png + char-carte-emissive.png');
