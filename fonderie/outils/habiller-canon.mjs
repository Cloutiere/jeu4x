// HABILLER-CANON — adaptation du canon Tripo (méca quadrupède) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-canon.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-canon.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-canon.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-canon.mjs              peint + exporte modeles/canon_v3.glb (+ cartes à plat)
//
// Source : image_ref/canon_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/canon_tripo.glb';
const SORTIE = '../modeles/canon_v3.glb';
...
const S = {
  ...
  teteY: [0.28, 0.55],
  teteXmax: 0.13,          // face/cabine du buste
  teteZmin: -0.18,
  tube: { y0: 0.28, x0: 0.12, zMax: -0.25 }, // canon rotatif d'épaule
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=cabine, LAME=canon rotatif
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/canon-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/canon-*.png');
  process.exit(0);
}

// lignes néon (cabine + canon) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : cabine + canon (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/canon-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/canon-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/canon-carte-base.png + canon-carte-emissive.png');
