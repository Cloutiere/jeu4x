// HABILLER-BOMBARDIER — adaptation du bombardier Tripo (aile volante) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-bombardier.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-bombardier.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-bombardier.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-bombardier.mjs              peint + exporte modeles/bombardier_v3.glb (+ cartes à plat)
//
// Source : image_ref/bombardier_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/bombardier_tripo.glb';
const SORTIE = '../modeles/bombardier_v3.glb';
...
const S = {
  ...
  teteY: [0.40, 0.59],
  teteXmax: 0.10,          // nez/cockpit (pointe avant)
  teteZmin: -0.05,
  tube: { y1: 0.18, x0: -0.16, x1: 0.16, z0: -0.10, z1: 0.20 }, // tentacules memétiques sous le fuselage
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=nez, LAME=tentacules/baie
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cy < S.tube.y1 && cx > S.tube.x0 && cx < S.tube.x1 && cz > S.tube.z0 && cz < S.tube.z1) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/bombardier-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/bombardier-*.png');
  process.exit(0);
}

// lignes néon (nez + tentacules) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : nez + tentacules (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/bombardier-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/bombardier-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/bombardier-carte-base.png + bombardier-carte-emissive.png');
