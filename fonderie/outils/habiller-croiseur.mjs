// HABILLER-CROISEUR — adaptation du croiseur Tripo (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-croiseur.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-croiseur.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-croiseur.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-croiseur.mjs              peint + exporte modeles/croiseur_v3.glb (+ cartes à plat)
//
// Source : image_ref/croiseur_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/croiseur_tripo.glb';
const SORTIE = '../modeles/croiseur_v3.glb';
...
const S = {
  ...
  lacetPos90: true,        // proue +X => -Z (confirmé --preview : batterie de canons à l'avant)
  axeFuselage: null,
  nivelerRoulis: false,
  ...
  teteY: [0.12, 0.20],
  teteXmax: 0.18,          // passerelle centrale
  teteZmax: 0.12,
  lame: { y0: 0.05, zBande: 0.35 }, // canons avant/arrière
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/croiseur-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/croiseur-*.png');
  process.exit(0);
}
...
writeFileSync(new URL('../captures/croiseur-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/croiseur-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/croiseur-carte-base.png + croiseur-carte-emissive.png');
