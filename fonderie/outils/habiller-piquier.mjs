// HABILLER-PIQUIER — adaptation du piquier Tripo (lance + bouclier au dos) (mode peintre : géométrie + UV conservées,
// texture repeinte + couche émissive, matériaux STYLE-3D). Session FONDERIE-LOT (mode peintre).
// Usage :
//   node outils/habiller-piquier.mjs --preview    rend 5 vues du source (détection du « avant »)
//   node outils/habiller-piquier.mjs --calibre    diagnostics géométriques (pieds, bandes Y, histogramme X)
//   node outils/habiller-piquier.mjs --zones      rend 5 vues colorées par zone + néon (vérif de la sélection)
//   node outils/habiller-piquier.mjs              peint + exporte modeles/piquier_v3.glb (+ cartes à plat)
//
// Source : image_ref/piquier_tripo.glb (5 492 tris, 6 965 sommets, UV, texture cuite,
...
const SOURCE = '../../image_ref/piquier_tripo.glb';
const SORTIE = '../modeles/piquier_v3.glb';
...
const S = {
  ...
  echelle: 3.7,          // source bas (0.70) : portée à 3.7 pour hauteur ≈ 2.6            // hauteur cible en unités de tuile (STYLE §5 ~2,5-2,75)
  ...
  teteY: [0.56, 0.71],
  teteXmax: 0.28,          // casque (face avant z<0)
  teteZmin: -0.05,
  lame: { y0: 0.34, y1: 0.52, xMin: 0.15, zMin: -0.15, zMax: 0.20 }, // lance (au-delà de la main)
...
// ---------- zones ----------
const Z = { CORPS: 0, VISIERE: 1, LAME: 2 }; // VISIERE=visière, LAME=lance
...
for (let t = 0; t < NB_TRIS; t++) {
  ...
  else if (cy > S.lame.y0 && cy < S.lame.y1 && cx > S.lame.xMin && cz > S.lame.zMin && cz < S.lame.zMax) zone[t] = Z.LAME;
}
...
if (process.argv.includes('--preview') || process.argv.includes('--zones')) {
  ...
  rendreVues({
    ...
    nomFichier: (v) => `../captures/piquier-${tri ? 'zones-' : ''}${v}.png`,
  });
  console.log('aperçus écrits dans captures/piquier-*.png');
  process.exit(0);
}

// lignes néon (visière + lance) en espace modèle pivoté/recentré, AVANT échelle
...
const materiaux = [
  ...
  { // 1 — néon pur : visière + lance (LINES) — JAMAIS teinté
    name: 'neon',
...
writeFileSync(new URL('../captures/piquier-carte-base.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteBase.buffer, carteBase.byteOffset, carteBase.length)));
writeFileSync(new URL('../captures/piquier-carte-emissive.png', import.meta.url), encoderPNG(RES, RES, Buffer.from(carteEmissive.buffer, carteEmissive.byteOffset, carteEmissive.length)));
console.log('cartes à plat : captures/piquier-carte-base.png + piquier-carte-emissive.png');
