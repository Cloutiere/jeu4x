...
2. **7k — Merveilles restantes & règles canoniques** — ✅ **handoff prêt : HANDOFF-PHASE7K.md** (04/09, avec Bloc 0 = corrections 7j d'Erik) : obsolescence **GLOBALE** (écart détecté : notre `isWonderObsolete` est par-owner), exclusivité mondiale, récupération des marteaux (fenêtre d'un tour 🔶 en tours simultanés), merveille = Jalon (vérifier), effets restants data-driven (Alexandrie, Shakespeare, Oxford seedé, Cie des Indes, Léonard, Foire de Troyes, Complexe militaro-industriel, Hollywood inactif, Apollo toutes techs, Internet, Grande Muraille validée), **audit des 8 merveilles actives contre le doc**, art dédiée des 6 classes GP ;
...
3. **Lancer la 7k** (HANDOFF-PHASE7K.md prêt, 04/09 — Bloc 0 corrections 7j d'Erik + merveilles canoniques) ; recueillir au passage ses votes de calibrage (§4 : liste 7i + 🔶 7j restants : multiplicateurs Settle additifs) et les consigner dans RULES.md/JSON.
