...
1. **Préalables** : lecture des docs normatifs, baseline (tests + typecheck verts, ~711 actuellement), `schemaVersion` courant (**16**), leçons récentes ;
...
5. **7n — Civilisations & traits** — ✅ **handoff prêt : HANDOFF-PHASE7N.md** (06/09, avec Bloc 0 = corrections 7m d'Erik) — **à lancer** : ères par comptage de techs (5/14/24, défaut à veto), `civilizations.json` 16 civs (départ + 4 bonus d'ère cumulatifs + unités uniques ~19 via pattern R-111), traits mappables implémentés / non mappables documentés inactifs (routes, caravanes, élite/Loyauté), sélection de civ au lobby (publique), migration 16→17 (`civId` neutre pour parties existantes) ;
6. **Phase Artefacts** (prochaine après 7n — Angkor Wat, Arche d'Alliance, École de Confucius, Atlantide ; doc « Merveilles et Personnages » §Distribution cartographique, couplée à la génération procédurale) ;
7. **Chantier visuel 3D** — Erik (06/09) : « tests sur la meilleure façon de le faire en fonction de la technologie » = **spike à cadrer** (2.5D isométrique via generate.py vs Three.js), après les artefacts ;
8. **Nouveau chantier — Migration thématique nanotechnologique** (Erik, 06/09) : une fois le jeu stabilisé, migration du monde historique vers un **monde virtuel nanotechnologique** — graphiques refaits **et renommage de toutes les technologies, unités, etc.** ; Erik a rédigé un rapport de recherche qu'il partagera (**pas encore dans le dépôt — le réclamer**) ;
9. **Espionnage avancé (BACKLOG.md idée 5)** — **en DERNIER**, après le thème ; Erik a déjà commandé le rapport de recherche 4X (résultat attendu à cette étape) ;
10. **Phase 8** — polish, équilibrage, esthétique (à replacer selon Erik, probablement avec le chantier visuel) ;
11. **En suspens d'Erik à cadrer** : territoire/frontières (prérequis du flip culturel et d'Hollywood), sauts technologiques. **Rapports de recherche encore attendus** : XP & promotions d'unité ; territoire & conversion culturelle ; **thème nanotechnologique (rédigé, à partager par Erik)**.
12. **Calibrage — décisions d'Erik du 06/09 (consignées, Bloc 0 du handoff 7n)** : ✅ ICBM — distinction canon **rétablie** (ville ordinaire rasée ; sa règle « ville survit » = capitale seulement) ; arrondi **⌈n/2⌉** ; pénalité **−1 jalon** validée ; vol d'or **50 %** validé ; matrice de duels OK pour l'instant (évoluera avec l'espionnage avancé) ; **Grande Muraille bloque l'ICBM** ; destruction de bâtiment = **choix avant l'action, coût et risque croissant avec la valeur du bâtiment** (défauts 🔶 : or = marteaux ×0,5, réussite = clamp(0,9 − marteaux/500 ; 0,4 ; 0,9), échec = espion + or perdus). **Encore en attente d'Erik** : 🔶 7l — facteur Industrielle ×5, sac de ville 0,5, classe des GP du canal or ; 🔶 7i — commerce du centre-ville par tranche ; 🔶 7n — cratère (permanent 🔶), villages→villes Mongols, merveille Égypte (choix au setup 🔶), stats unités uniques à auditer, ère par comptage (défaut à veto).
