...
- **L1b — Portage de la spec visuelle** : le `design.js` du prototype devient une **spec déclarative data-driven** (`assets-src` ou `packages/rules/data/visuel.json` 🔶 — couleurs, élévations, glyphes, **matériaux** par terrain calibrables sans code) ; intégration du pipeline `generate.py`/`sync-art` si des assets 3D sont nécessaires ;
