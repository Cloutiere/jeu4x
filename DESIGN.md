...
| **3 — Rendu & UI** *(handoff prêt : HANDOFF-PHASE3.md)* | On joue à la souris sur une partie locale (bot aléatoire) | Canvas PixiJS (caméra zoom/pan/culling) ; spritesheets **placeholder géométriques** ; stores Svelte ← messages réseau ; barre sup., panneau d'unité, menu de ville, fin de tour ; playback des événements |
...
| **7 — Contenu Civ Revolution** | Le vrai jeu | Arbre technologique complet, unités (dont **unités à distance**, R-59), merveilles, grandes personnes, **diplomatie** (paix/guerre, incidents, restitution de colons — R-58) — data-driven |
| **Parallèle — Atelier sprites** | Tileset + unités cohérents | Démarré dès la Phase 2 (ne bloque que le rendu) ; sources dans `/assets-src`, licences consignées |
