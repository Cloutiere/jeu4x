...
| **4 — Prototype J1 complet** *(handoff : HANDOFF-PHASE4.md)* | 2 humains finissent une partie 1v1 **en ligne** (déploiement réel) | Déploiement Cloudflare + OAuth réels (prérequis utilisateur : compte + apps OAuth) ; durcissement multi-attaquants (BACKLOG idée 2, test) ; polish UX relevé à l'acceptation Phase 3 (bouton Fonder désactivé près d'une ville, refus de case finale alliée, toast d'ordre non exécuté) ; calibration T-15 (croissance trop rapide observée) |
...
| **6 — Génération procédurale** | Cartes aléatoires équitables | Bruit + placement symétrique des spawns, checksum d'équité, seeds rejouables |
| **7 — Contenu Civ Revolution** | Le vrai jeu | Arbre technologique complet, unités (dont **unités à distance**, R-59), merveilles, grandes personnes, **diplomatie** (paix/guerre, incidents, restitution de colons — R-58), **engagements multi-participants** (BACKLOG.md idée 2, §7.9 à formaliser) — data-driven |
