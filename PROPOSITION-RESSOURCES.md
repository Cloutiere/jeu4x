...
| # | Question | Décision |
|---|---|---|
| **D1** | Visibilité d'une ressource non débloquée | ✅ **Révélation avec la technologie** — nouvelle adaptation assumée, **diffère du jeu original** (CivRev affiche les icônes dès l'exploration, cf. recherche §3) : une ressource `revealedByTech` non débloquée est **invisible** pour le joueur. `hiddenUntilRevealed: true` par défaut en données. **Révisée le 01/09 pendant l'implémentation** : la **présence** d'une ressource reste toujours visible — l'état filtré diffuse le **marqueur « inconnue »** (icône « ? ») tant que la tech manque, et l'**identité réelle** apparaît au déblocage ; le bonus reste verrouillé (R-92 révisée, commit `c73c44b`). La variante CivRev-fidèle reste en données (`hiddenUntilRevealed: false` → icône réelle d'office, bonus verrouillé). |
| **D2** | Culture | ✅ **No-go moteur, données prêtes** — `resources.json` porte `culture` (Encens 2, Soie 3), ignoré par le moteur tant que le système (grandes personnes/temples) n'est pas acté. Conséquence assumée : Encens/Soie n'ont aucun effet mécanique en v1 (aucun rendement N/P/C officiel — exception « OU culture > 0 » au test d'intégrité R-91). |
