...
describe('catalogue data-driven type → modèle 3D (visuel3d.json, fonderie T3)', () => {
  ...
  it('les .glb servis sont TOUS mappés (24 fichiers, barbare_v3 via la surcharge propriétaire)', () => {
    ...
    const mappes = [
      ...
      ...(TUILE_COLLINE3D && TUILE_COLLINE3D.kind === 'glb' ? [TUILE_COLLINE3D.glb.replace('_v3', '_v2')] : []),
      ...(TUILE_COLLINE3D && TUILE_COLLINE3D.kind === 'glb' ? [TUILE_COLLINE3D.glb.replace('_v3', '_v1')] : []),
      // v1/v2 des tuiles servies en parallèle des v3 (même contenu corrigé, caches edge)
